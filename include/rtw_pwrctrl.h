...
#define MAX_SNMP_COMMUNITY_NAME_NUM     2
#define MAX_SNMP_DISPLAY_STRING_LEN     32
#define MAX_SNMP_OBJECT_ID_LEN          8
#define MAX_SNMP_OBJECT_NAME_LEN        8

#ifdef CONFIG_WAKE_PATTERN_EN

#define MAX_WAKE_SIZE	16 /* (16 bytes for WKFM bit mask, 16*8 = 128 bits) */
#define MAX_WAKE_PATTERN_SIZE	128
#define MAX_WAKE_NUM	5

#if defined(CONFIG_RTL8822B)
#define MAX_TOTAL_WAKE_PATTERN_SIZE (128 * MAX_WAKE_NUM - PACKET_OFFSET_SZ) /* rsvd page size * 2 - PACKET_OFFSET_SZ */
#endif 

typedef struct rtl_wake_pattern {
	u16 	len;
	u16 	len_total;
	char 	content[MAX_WAKE_PATTERN_SIZE];
	char 	mask[MAX_WAKE_SIZE];
} rtl_wake_pattern_t;

struct wow_wake_pattern_priv {
	u8	wowlan_wake_pattern_idx;	/* Max: MAX_WAKE_NUM */
	u16 wowlan_wake_total_len;		/* Max: MAX_TOTAL_UNWAKE_PATTERN_SIZE */
	struct rtl_wake_pattern wake_patterns[MAX_WAKE_NUM];
};
...
struct war_netbios_fmt {
    u8 ntbname[MAX_NETBIOS_NAME_LEN+1];  /* as string */
    u8 ntbname_len;
	u8 ntbname_encoded[30];
};

struct netbios_char_mapping{
	u8 target_char;
    u8 target_ascii[2];
};

#if defined(CONFIG_OFFLOAD_SSDP_V4) || defined(CONFIG_OFFLOAD_SSDP_V6)
#define MAX_SSDP_PATTERN_SIZE	128
#define MAX_SSDP_NUM	10
#define MAX_TOTAL_SSDP_PATTERN_SIZE 128 * 2 /* rsvd page size  */
typedef struct rtl_ssdp_pattern {
	u16 	len;
	char 	content[MAX_SSDP_PATTERN_SIZE];
} rtl_ssdp_pattern_t;

struct wow_ssdp_pattern_priv {
	u8	wowlan_ssdp_pattern_idx;	/* Max: 5 */
	u16	wowlan_ssdp_total_len;	/* Max: 128 * 2*/
	struct rtl_ssdp_pattern ssdp_patterns[MAX_SSDP_NUM];
};
...
#define MAX_WSD_PATTERN_SIZE	128
#define MAX_WSD_NUM	10
#define MAX_TOTAL_WSD_PATTERN_SIZE 128 * 2 /* rsvd page size  */
typedef struct rtl_wsd_pattern {
	u16 	len;
	char 	content[MAX_WSD_PATTERN_SIZE];
} rtl_wsd_pattern_t;

struct wow_wsd_pattern_priv {
	u8	wowlan_wsd_pattern_idx;	/* Max: 5 */
	u16	wowlan_wsd_total_len;	/* Max: 128 * 2*/
	struct rtl_wsd_pattern wsd_patterns[MAX_WSD_NUM];
};
...
#define MAX_SLP_PATTERN_SIZE	128
#define MAX_SLP_NUM	10
#define MAX_TOTAL_SLP_PATTERN_SIZE 128 * 2  /* rsvd page size  */
typedef struct rtl_slp_pattern {
	u16 	len;
	char 	content[MAX_SLP_PATTERN_SIZE];
} rtl_slp_pattern_t;

struct wow_slp_pattern_priv {
	u8	wowlan_slp_pattern_idx;	/* Max: 5 */
	u16	wowlan_slp_total_len;	/* Max: 128 * 2*/
	struct rtl_slp_pattern slp_patterns[MAX_SLP_NUM];
};
...
struct pwrctrl_priv {
	...
	u8		wowlan_lps_1t1r;
		...
		struct war_netbios_fmt wowlan_war_offload_netbios;
		u8		wowlan_war_offload_snmp_community_name[MAX_SNMP_COMMUNITY_NAME_NUM][MAX_SNMP_COMMUNITY_NAME_LEN+1];
		u8		wowlan_war_offload_snmp_community_name_len[MAX_SNMP_COMMUNITY_NAME_NUM];
		u8		wowlan_war_offload_snmp_device_status;
		u8		wowlan_war_offload_snmp_printer_status;
		u8		wowlan_war_offload_snmp_sleep_status;
		u16 	wowlan_war_offload_snmp_error_status;
		u8		wowlan_war_offload_snmp_display_string[MAX_SNMP_DISPLAY_STRING_LEN+1];
		u8		wowlan_war_offload_snmp_display_string_len;
		u16 	wowlan_war_offload_snmp_enterprise_id;
		u8		wowlan_war_offload_snmp_enterprise_id_buf[3];
		u8		wowlan_war_offload_snmp_object_id[MAX_SNMP_OBJECT_ID_LEN+1];
		u8		wowlan_war_offload_snmp_object_id_len;
		u8		wowlan_war_offload_snmp_object_name[MAX_SNMP_OBJECT_NAME_LEN+1];
		u8		wowlan_war_offload_snmp_object_name_len;
#ifdef CONFIG_WAKE_PATTERN_EN	
		struct wow_wake_pattern_priv wpriv;
#endif /* CONFIG_WAKE_PATTERN_EN */
	
#if defined(CONFIG_OFFLOAD_SSDP_V4) || defined(CONFIG_OFFLOAD_SSDP_V6)
		struct	wow_ssdp_pattern_priv ssdppriv;
#endif /* defined(CONFIG_OFFLOAD_SSDP_V4) || defined(CONFIG_OFFLOAD_SSDP_V6) */
#if defined(CONFIG_OFFLOAD_WSD_V4) || defined(CONFIG_OFFLOAD_WSD_V6)
		struct	wow_wsd_pattern_priv wsdpriv;
#endif /* defined(CONFIG_OFFLOAD_WSD_V4) || defined(CONFIG_OFFLOAD_WSD_V6) */
#if defined(CONFIG_OFFLOAD_SLP_V4) || defined(CONFIG_OFFLOAD_SLP_V6)
		struct	wow_slp_pattern_priv slppriv;
#endif /* defined(CONFIG_OFFLOAD_SLP_V4) || defined(CONFIG_OFFLOAD_SLP_V6) */
		...
		u8	   wowlan_war_offload_llmnr_data[MAX_LLMNR_DATA_LEN+1];
		u8	   wowlan_war_offload_llmnr_data_len;
#endif
#endif /* CONFIG_WAR_OFFLOAD */
#endif /* CONFIG_WOWLAN */
...
bool rtw_wow_wake_pattern_parser_pattern_cmd(u8 *input, char *pattern, u16 *pattern_len, char *bit_mask);
