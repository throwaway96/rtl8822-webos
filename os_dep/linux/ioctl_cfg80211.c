				...
				struct cfg80211_connect_params *sme)
{
...
#ifdef LGE_PRIVATE
	{
		...
		if (!(rfctl && rfctl->country_ent)) {
			ret = -EBUSY;
			RTW_INFO("%s Country domain don't initialized.\n", __func__);
			goto exit;
		}
	}
#endif
		...
		struct vif_params *params)
{
	...
	case NL80211_IFTYPE_MESH_POINT:
		...
		if ((type == NL80211_IFTYPE_P2P_CLIENT || type == NL80211_IFTYPE_P2P_GO)
			#if (defined(RTW_P2P_GROUP_INTERFACE))
			&& 0
			#else
	...
	unsigned int duration, u64 *cookie)
{
	...
	*cookie = ATOMIC_INC_RETURN(&pcfg80211_wdinfo->ro_ch_cookie_gen);
	...
	if (rtw_mi_check_fwstate(padapter, WIFI_UNDER_SURVEY) &&
		rtw_mi_buddy_check_fwstate(padapter, WIFI_UNDER_SURVEY)) {
		err = -EBUSY;
		goto exit;
	}
	...
	if (rtw_mi_buddy_check_fwstate(padapter, WIFI_ASOC_STATE)) {
		u8 op_ch = rtw_get_oper_ch(padapter);

		if ((remain_ch != op_ch) && !(op_ch > 48 && op_ch < 149)) {
...
struct ieee80211_iface_combination rtw_combinations[] = {
	{
		...
		#if defined(CONFIG_MCC_MODE) && defined(LGE_PRIVATE)
		.num_different_channels = 3,
		#elif defined(CONFIG_MCC_MODE)
...
#if defined(LGE_PRIVATE) && defined(PURE_SUPPLICANT)
