...
char *rtw_ext_path2 = "/mnt/lg/res/lglib/CcodeTable_RT8822";
module_param(rtw_ext_path2, charp, 0644);
MODULE_PARM_DESC(rtw_ext_path2, "External Network Setting");

/* PHY_FILE_TXPWR_LMT */
char *rtw_ext_path3 = "/mnt/lg/res/lglib/TxPwrLimit_RT8822.dat";
module_param(rtw_ext_path3, charp, 0644);
MODULE_PARM_DESC(rw_ext_path3, "External Network Setting");
...
char *rtw_ext_path5 = "/mnt/platform-plugins/conf/CcodeTable_RT8822";
module_param(rtw_ext_path5, charp, 0644);
MODULE_PARM_DESC(rtw_ext_path5, "External Network Setting");

char *rtw_ext_path6 = "/mnt/platform-plugins/conf/TxPwrLimit_RT8822.dat";
	...
	for (i = 0; i < dvobj->iface_nums; i++) {
		...
		if (adapter && !adapter->pnetdev) {
			...
			if (!is_primary_adapter(adapter) &&
				(i >= (CONFIG_IFACE_NUMBER - CONFIG_RTW_DYNAMIC_NDEV)))
				continue;
			#endif
...
#ifdef LGE_PRIVATE
{
	...
	if (hal_data->txpwr_limit_from_file == 0) {
		/* will not happen */
		LGE_MSG("[WLAN] TxPwrLimit is not exist in %s", REALTEK_CONFIG_PATH);
		LGE_MSG("[WLAN] Load default TxPwrLimit");
	}
	...
	if (load_default_table == 1) {
		...
		if (ret != 0) {
			/* will not happen */
			if (ret == -2) {
				LGE_MSG("[WLAN] No match PowerSetting on CcodeTable in %s", REALTEK_CONFIG_PATH);
			}
			/* apply initial fail */
...
#ifdef LGE_PRIVATE
	/* ret : 0: normal; -1: fail */
	if ((ret == 0) && !RTW_CANNOT_RUN(padapter)) {
		...
		if(registry_par->suspend_type == FW_IPS_DISABLE_BBRF && !check_fwstate(pmlmepriv, WIFI_ASOC_STATE)) {
			if (!rtw_is_surprise_removed(padapter)) {
				...
				if (rtw_hal_init(padapter) == _FAIL)
					RTW_INFO("%s hal intial fail!", __func__);
#ifdef LGE_PRIVATE
				else
					padapter->hal_func.hal_init_post(padapter);
#endif
