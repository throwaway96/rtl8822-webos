...
u8 mcc_switch_channel_policy_table[][7]={
	{30, 50, 35, 100, 3, 0, 30}, /* favor P2P */
	{70, 50, 15, 100, 3, 0, 30}, /* favor STA */
	{40, 50, 30, 100, 3, 0, 30}, /* fair scheduling */
	{20, 50, 40, 100, 3, 0, 30}, /* fair scheduling, favor P2P */
	{80, 50, 10, 100, 3, 0, 30}, /* fair scheduling, favor STA */
	{30, 50, 35, 100, 3, 0, 30},
	{40, 50, 30, 100, 3, 0, 30}, /* 6 */
	{50, 50, 25, 100, 3, 0, 30},
	{60, 50, 20, 100, 3, 0, 30},
	{70, 50, 15, 100, 3, 0, 30}, /* 9 */
	{80, 50, 10, 100, 3, 0, 30},
	{90, 50,  5, 100, 3, 0, 30},
	{80, 75, 35, 150, 3, 0, 20},
};
...
static void mcc_halrf_dpk_switch(_adapter *adapter, u8 enable)
{
	struct dm_struct *dm = NULL;

	dm = adapter_to_phydm(adapter);
	halrf_dpk_switch(dm, enable);
}
...
static void rtw_hal_mcc_cfg_phydm (_adapter *adapter, enum mcc_cfg_phydm_ops ops, void *data)
{
	...
	case MCC_CFG_PHYDM_DUMP:
		...
		break;
	case MCC_CFG_DPK_SWITCH_OFF:
		mcc_halrf_dpk_switch(adapter, _FALSE);
		break;
	case MCC_CFG_DPK_SWITCH_ON:
		mcc_halrf_dpk_switch(adapter, _TRUE);
		break;
			...
			*index += (CurtPktPageNum * page_size);
			...
			for (path = RF_PATH_A; path < hal->NumTotalRFPath; ++path) {
				...
				if (ch <= 14) {
					...
					rates = rates_by_sections[CCK].rates;
					for (j = 0; j < rate_array_sz; ++j) {
						power_index = phy_get_tx_power_index(iface, path, rates[j], bw, center_ch);
						rate = PHY_GetRateIndexOfTxPowerByRate(rates[j]);
...
static void rtw_hal_mcc_start_prehdl(PADAPTER padapter)
{
	...
	rtw_hal_mcc_cfg_phydm(padapter, MCC_CFG_DPK_SWITCH_OFF, NULL);
}
...
static void rtw_hal_mcc_stop_posthdl(PADAPTER padapter)
{
	...
	rtw_hal_mcc_cfg_phydm(padapter, MCC_CFG_DPK_SWITCH_ON, NULL);
	#endif
	hal->multi_ch_switch_mode = _FALSE;
}
...
static void rtw_hal_mcc_start_posthdl(PADAPTER padapter)
{
	...
	#endif
	hal->multi_ch_switch_mode = _TRUE;
	...
	if (MCC_EN(padapter)) {
		...
		if (MSTATE_STA_LD_NUM(&mstate) || MSTATE_STA_LG_NUM(&mstate) || MSTATE_AP_NUM(&mstate)) {
			...
			if (chbw_allow == _FALSE) {
				...
				_exit_critical_mutex(&pmccobjpriv->mcc_mutex, NULL);
				if (ret == _FAIL) {
					/* if MCC setting fail, the station will be disconnect */
					rtw_hal_set_mcc_status(padapter, MCC_STATUS_NEED_MCC | MCC_STATUS_DOING_MCC);
...
static u8 set_mcc_duration_hdl(PADAPTER adapter, const u8 *val)
{
	...
	if (type == MCC_DURATION_MAPPING) {
		switch (duration) {
			...
			case 5:
				mccobjpriv->duration = 30;
				mccobjpriv->policy_index = 5;
				mccobjpriv->mchan_sched_mode = MCC_FAVOR_P2P;
				break;
			case 6:
				mccobjpriv->duration = 40;
				mccobjpriv->policy_index = 6;
				mccobjpriv->mchan_sched_mode = MCC_FAVOR_P2P;
				break;
			case 7:
				mccobjpriv->duration = 50;
				mccobjpriv->policy_index = 7;
				mccobjpriv->mchan_sched_mode = MCC_FAVOR_P2P;
				break;
			case 8:
				mccobjpriv->duration = 60;
				mccobjpriv->policy_index = 8;
				mccobjpriv->mchan_sched_mode = MCC_FAVOR_P2P;
				break;
			case 9:
				mccobjpriv->duration = 70;
				mccobjpriv->policy_index = 9;
				mccobjpriv->mchan_sched_mode = MCC_FAVOR_P2P;
				break;
			case 10:
				mccobjpriv->duration = 80;
				mccobjpriv->policy_index = 10;
				mccobjpriv->mchan_sched_mode = MCC_FAVOR_P2P;
				break;
			case 11:
				mccobjpriv->duration = 90;
				mccobjpriv->policy_index = 11;
				mccobjpriv->mchan_sched_mode = MCC_FAVOR_P2P;
				break;
			case 12:
				mccobjpriv->duration = 80;
				mccobjpriv->policy_index = 12;
				mccobjpriv->mchan_sched_mode = MCC_FAVOR_P2P;
				break;
			default:
				mccobjpriv->duration = 30;
