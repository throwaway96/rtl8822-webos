...
static u8 rtw_hal_set_war_offload_parm(_adapter *adapter, PRSVDPAGE_LOC rsvdpageloc)
{
	...
	SET_H2CCMD_WAROFLD_RSVDPAGE1_LOC_ICMP_PARM(u1H2CWarOfldParm, rsvdpageloc->LocIpParm);
	...
	if (pwrctl->wowlan_pno_enable == _FALSE) {
		...
		*total_pkt_len = index + (page_size * CurtPktPageNum);
		...
		if (psecpriv->binstallBIPkey) {
			...
			_rtw_memcpy(pframe + index - tx_desc + 8 + 2 + 6 + 8, psecpriv->dot11wBIPKey[4].skey, 32);
			_rtw_memcpy(pframe + index - tx_desc + 8 + 2 + 6 + 8 + 32, psecpriv->dot11wBIPKey[5].skey, 32);
				...
				if(_TRUE == pwrctl->wowlan_war_offload_mode) {
					...
					*total_pkt_len = index + (page_size * CurtPktPageNum);
					...
					if (WAR_MDNS_V6_RSP_EN & pwrctl->wowlan_war_offload_ctrl) {
						...
						rtw_hal_construct_mdns_rsp_v6(adapter, &pframe[index], &buf_len, pmlmeinfo->ip6_addr);
						rtw_hal_fill_fake_txdesc(adapter, &pframe[index - tx_desc], buf_len, _FALSE, _FALSE, _TRUE);
						CurtPktPageNum = 16;
						*page_num += CurtPktPageNum;
						index += (CurtPktPageNum * page_size);
...
static void rtw_hal_wow_enable(_adapter *adapter)
{
	...
	/* don't generate usb toogle signal during suspend process, for 8822c*/
...
#else
		cam_id = rtw_get_max_mbid_cam_id(adapter) + 1;
#endif
		if (cam_id != INVALID_CAM_ID)
...
#ifdef CONFIG_MCC_MODE
	/*======== MCC * n page ======== */
	if ((is_wow_mode == _FALSE) && MCC_EN(adapter)) {/*Normal mode*/
	...
	case HW_VAR_HCI_SUS_STATE:
		...
		case HW_VAR_BCN_HEAD_SEL:
		{
			...
			if (MLME_IS_AP(adapter) || MLME_IS_MESH(adapter)) {
				...
				RTW_DBG(ADPT_FMT" vap_id(%d) change BCN HEAD to 0x%04x\n",
			...
			while ((rtw_end_of_queue_search(phead, plist)) == _FALSE) {
				...
				if (psta) {
					...
					if ((_rtw_memcmp(psta->cmn.mac_addr, bc_addr, 6) !=   _TRUE)
						...
						/* txpower, qdBm = 4 * dbm */
						current_txpwr = 4 * (rtw_adapter_get_oper_txpwr_max_mbm(padapter) / MBM_PDBM);
