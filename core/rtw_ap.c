...
#endif
#ifdef LGE_PRIVATE
	if (rtw_mi_buddy_check_fwstate(padapter, WIFI_UNDER_LINKING | WIFI_UNDER_SURVEY))
		return;
#endif
		...
		for (i = 0; i < chk_alive_num; i++) {
			...
			if (pmlmeext->active_keep_alive_check) {
				...
				if (ret == _SUCCESS) {
					...
					psta->keep_alive_trycnt = 0;
					continue;
				} else if (psta->keep_alive_trycnt <= 6) {
...
int rtw_check_beacon_data(_adapter *padapter, u8 *pbuf,  int len)
{
	...
	if (padapter->registrypriv.ht_enable &&
		...
		if (MLME_IS_GO(padapter)) {
			...
			if (cbw40_enable)
				valid = rtw_get_offset_by_chbw(pbss_network->Configuration.DSConfig, CHANNEL_WIDTH_40, &ch_offset);


			if (cbw40_enable && valid && ch_offset == HAL_PRIME_CHNL_OFFSET_LOWER) {
			...
			} else if (cbw40_enable && valid && ch_offset == HAL_PRIME_CHNL_OFFSET_UPPER) {
				SET_HT_CAP_ELE_CHL_WIDTH(pHT_caps_ie + 2, 1);
				SET_HT_OP_ELE_STA_CHL_WIDTH(pHT_info_ie + 2, 1);
