...
int check_ielen(u8 *start, uint len)
{
	...
	while (left >= 2) {
		...
		if ((id == WLAN_EID_VENDOR_SPECIFIC) &&
			#ifdef LGE_PRIVATE
			/* Korea test lab, Athene's Beacon include invalid vendor IE length 0 */
			(elen != 0) &&
			#endif
			(elen < 3))
				return _FALSE;
...
void rtw_wow_ssdp_pattern_sw_reset(_adapter *adapter)
{
	int i;
	struct pwrctrl_priv *pwrctrlpriv = adapter_to_pwrctl(adapter);

	pwrctrlpriv->ssdppriv.wowlan_ssdp_pattern_idx = 0;
	/* First byte of total pattern content would be total pattern num */
	pwrctrlpriv->ssdppriv.wowlan_ssdp_total_len = 1;

	for (i = 0 ; i < MAX_SSDP_NUM; i++) {
		_rtw_memset(pwrctrlpriv->ssdppriv.ssdp_patterns[i].content, '\0', MAX_SSDP_PATTERN_SIZE);
		pwrctrlpriv->ssdppriv.ssdp_patterns[i].len = 0;
	}
}
bool rtw_wow_ssdp_pattern_parser_pattern_cmd(u8 *input, unsigned long input_len, char *pattern, u16 *pattern_len)
{
	char *cp = NULL;
	size_t len = 2;
	int pos = 0, mask_pos = 0, res = 0;
	...
	/* reset */
	_rtw_memset(pattern, '\0', MAX_SSDP_PATTERN_SIZE);
	(*pattern_len) = 0;

	if(input_len <= MAX_SSDP_PATTERN_SIZE) {
		for(idx = 0; idx < input_len; idx++) {
			if(_rtw_memcmp(&lf, input+idx, 1)) 
				break;
			_rtw_memcpy(pattern+idx, input+idx, 1);
			(*pattern_len)++;
		}
	}
	else {
		RTW_ERR("%s:[ERROR] pattern length is invalid!!(%lu)\n",__func__, input_len);
		goto error;
	}	
...
bool rtw_wow_ssdp_get_pattern_content(_adapter *adapter, u8 *pcur_pattern, u32 *PatternLength)
{
	struct pwrctrl_priv *pwrpriv = adapter_to_pwrctl(adapter);
	struct wow_ssdp_pattern_priv *pssdppriv = &pwrpriv->ssdppriv;
	int k = 0, pos = 0, len_mask = 0;
	int cur_pattern_num = 0;

	_rtw_memset(pcur_pattern, 0, MAX_TOTAL_SSDP_PATTERN_SIZE);

	*PatternLength = pssdppriv->wowlan_ssdp_total_len ;
	cur_pattern_num = pssdppriv->wowlan_ssdp_pattern_idx;

	/* 0: Total pattern number */
	_rtw_memcpy(pcur_pattern + pos, &pssdppriv->wowlan_ssdp_pattern_idx, sizeof(pssdppriv->wowlan_ssdp_pattern_idx));
	pos += sizeof(pssdppriv->wowlan_ssdp_pattern_idx);;
	
	for (k = 0; k < cur_pattern_num ; k++) {

		/* 1: pattern length */
		_rtw_memcpy(pcur_pattern + pos, &pssdppriv->ssdp_patterns[k].len, sizeof(pssdppriv->ssdp_patterns[k].len));
		pos += sizeof(pssdppriv->ssdp_patterns[k].len);

		/* 2: pattern content */
		_rtw_memcpy(pcur_pattern + pos, pssdppriv->ssdp_patterns[k].content, pssdppriv->ssdp_patterns[k].len);
		pos += pssdppriv->ssdp_patterns[k].len;
	}

	if(*PatternLength != pos) 
		RTW_INFO("%s: Length don't match !(%d/%d)\n", __func__, *PatternLength, pos);


	return TRUE;
}

int rtw_wow_ssdp_get_pttern_len(int pattern_content_len)
{
        /* length field + content field */
	return (2 + pattern_content_len);
}

int rtw_wow_ssdp_check_pattern_len(_adapter *adapter)
{
	struct pwrctrl_priv *pwrpriv = adapter_to_pwrctl(adapter);	
	struct wow_ssdp_pattern_priv *pssdppriv = &pwrpriv->ssdppriv;
	int index = 0, pattern_len = 0;

	index = pssdppriv->wowlan_ssdp_pattern_idx ;
	pattern_len = rtw_wow_ssdp_get_pttern_len(pssdppriv->ssdp_patterns[index].len);

	/* RTW_INFO("%s: pattetn %d, all pattern %d, total patterns %d\n", __func__, 
		pssdppriv->ssdp_patterns[index].len, pattern_len, (pssdppriv->wowlan_ssdp_total_len + pattern_len)); */

	if( pssdppriv->ssdp_patterns[index].len > MAX_SSDP_PATTERN_SIZE)
	{
		RTW_PRINT("WARNING: the length of input pattern (%d) is greater than %d.\n", 
						pattern_len, MAX_SSDP_PATTERN_SIZE);
		return PATTERN_ERROR_EXCEEDING_PATTERN_LEN; 
	}	
	else if((pssdppriv->wowlan_ssdp_total_len + pattern_len) > MAX_TOTAL_SSDP_PATTERN_SIZE)
	{
		RTW_PRINT("WARNING: the length of total patterns (%d) is greater than %d.\n", 
						(pssdppriv->wowlan_ssdp_total_len + pattern_len), MAX_TOTAL_SSDP_PATTERN_SIZE);
		return PATTERN_ERROR_EXCEEDING_TOTAL_PATTERN_LEN;
	}	
	else 
		return pattern_len;
...
void rtw_wow_wsd_pattern_sw_reset(_adapter *adapter)
{
	int i;
	struct pwrctrl_priv *pwrctrlpriv = adapter_to_pwrctl(adapter);

	pwrctrlpriv->wsdpriv.wowlan_wsd_pattern_idx = 0;
	/* First byte of total pattern content would be total pattern num */
	pwrctrlpriv->wsdpriv.wowlan_wsd_total_len = 1;

	for (i = 0 ; i < MAX_WSD_NUM; i++) {
		_rtw_memset(pwrctrlpriv->wsdpriv.wsd_patterns[i].content, '\0', MAX_WSD_PATTERN_SIZE);
		pwrctrlpriv->wsdpriv.wsd_patterns[i].len = 0;
	}
}
bool rtw_wow_wsd_pattern_parser_pattern_cmd(u8 *input, unsigned long input_len, char *pattern, u16 *pattern_len)
{
	char *cp = NULL;
	size_t len = 2;
	int pos = 0, mask_pos = 0, res = 0;
	...
	/* reset */
	_rtw_memset(pattern, '\0', MAX_WSD_PATTERN_SIZE);
	(*pattern_len) = 0;

	if(input_len <= MAX_WSD_PATTERN_SIZE) {
		for(idx = 0; idx < input_len; idx++) {
			if(_rtw_memcmp(&lf, input+idx, 1)) 
				break;
			_rtw_memcpy(pattern+idx, input+idx, 1);
...
bool rtw_wow_wsd_get_pattern_content(_adapter *adapter, u8 *pcur_pattern, u32 *PatternLength)
{
	struct pwrctrl_priv *pwrpriv = adapter_to_pwrctl(adapter);
	...
	_rtw_memset(pcur_pattern, 0, MAX_TOTAL_WSD_PATTERN_SIZE);

	*PatternLength = pwsdpriv->wowlan_wsd_total_len ;
	cur_pattern_num = pwsdpriv->wowlan_wsd_pattern_idx;
	
	/* 0: Total pattern number */
	_rtw_memcpy(pcur_pattern + pos, &pwsdpriv->wowlan_wsd_pattern_idx, sizeof(pwsdpriv->wowlan_wsd_pattern_idx));
	pos += sizeof(pwsdpriv->wowlan_wsd_pattern_idx);
	
	for (k = 0; k < cur_pattern_num ; k++) {

		/* 1: pattern length */
		_rtw_memcpy(pcur_pattern + pos, &pwsdpriv->wsd_patterns[k].len, sizeof(pwsdpriv->wsd_patterns[k].len));
		pos += sizeof(pwsdpriv->wsd_patterns[k].len);

		/* 2: pattern content */
		_rtw_memcpy(pcur_pattern + pos, pwsdpriv->wsd_patterns[k].content, pwsdpriv->wsd_patterns[k].len);
		pos += pwsdpriv->wsd_patterns[k].len;
	}
...
int rtw_wow_wsd_get_pttern_len(int pattern_content_len)
{
        /* length field + content field */
	return (2 + pattern_content_len);
}

int rtw_wow_wsd_check_pattern_len(_adapter *adapter)
{
	struct pwrctrl_priv *pwrpriv = adapter_to_pwrctl(adapter);	
	struct wow_wsd_pattern_priv *pwsdpriv = &pwrpriv->wsdpriv;
	int index = 0, pattern_len = 0;
	...
	pattern_len = rtw_wow_wsd_get_pttern_len(pwsdpriv->wsd_patterns[index].len);

	/* RTW_INFO("%s: pattetn %d, all pattern %d, total patterns %d\n", __func__, 
		pwsdpriv->wsd_patterns[index].len, pattern_len, (pwsdpriv->wowlan_wsd_total_len + pattern_len)); */

	if( pwsdpriv->wsd_patterns[index].len > MAX_WSD_PATTERN_SIZE)
	{
		RTW_PRINT("WARNING: the length of input pattern (%d) is greater than %d.\n", 
						pattern_len, MAX_WSD_PATTERN_SIZE);
		return PATTERN_ERROR_EXCEEDING_PATTERN_LEN; 
	}	
	else if((pwsdpriv->wowlan_wsd_total_len + pattern_len) > MAX_TOTAL_WSD_PATTERN_SIZE)
	{
		RTW_PRINT("WARNING: the length of total patterns (%d) is greater than %d.\n", 
						(pwsdpriv->wowlan_wsd_total_len + pattern_len), MAX_TOTAL_WSD_PATTERN_SIZE);
		return PATTERN_ERROR_EXCEEDING_TOTAL_PATTERN_LEN;
	}	
	else 
		return pattern_len;
...
void rtw_wow_slp_pattern_sw_reset(_adapter *adapter)
{
	int i;
	struct pwrctrl_priv *pwrctrlpriv = adapter_to_pwrctl(adapter);

	pwrctrlpriv->slppriv.wowlan_slp_pattern_idx = 0;
	/* First byte of total pattern content would be total pattern num */
	pwrctrlpriv->slppriv.wowlan_slp_total_len = 1;

	for (i = 0 ; i < MAX_SLP_NUM; i++) {
		_rtw_memset(pwrctrlpriv->slppriv.slp_patterns[i].content, '\0', MAX_SLP_PATTERN_SIZE);
		pwrctrlpriv->slppriv.slp_patterns[i].len = 0;
	}
}
bool rtw_wow_slp_pattern_parser_pattern_cmd(u8 *input, unsigned long input_len, char *pattern, u16 *pattern_len)
{
	char *cp = NULL;
	size_t len = 2;
	int pos = 0, mask_pos = 0, res = 0;
	...
	/* reset */
	_rtw_memset(pattern, '\0', MAX_SLP_PATTERN_SIZE);
	(*pattern_len) = 0;

	if(input_len <= MAX_SLP_PATTERN_SIZE) {
		for(idx = 0; idx < input_len; idx++) {
			if(_rtw_memcmp(&lf, input+idx, 1)) 
				break;
			_rtw_memcpy(pattern+idx, input+idx, 1);
...
bool rtw_wow_slp_get_pattern_content(_adapter *adapter, u8 *pcur_pattern, u32 *PatternLength)
{
	struct pwrctrl_priv *pwrpriv = adapter_to_pwrctl(adapter);
	...
	_rtw_memset(pcur_pattern, 0, MAX_TOTAL_SLP_PATTERN_SIZE);

	*PatternLength = pslppriv->wowlan_slp_total_len ;
	cur_pattern_num = pslppriv->wowlan_slp_pattern_idx;

	/* 0: Total pattern number */
	_rtw_memcpy(pcur_pattern + pos, &pslppriv->wowlan_slp_pattern_idx, sizeof(pslppriv->wowlan_slp_pattern_idx));
	pos += sizeof(pslppriv->wowlan_slp_pattern_idx);
	
	for (k = 0; k < cur_pattern_num ; k++) {

		/* 1: pattern length */
		_rtw_memcpy(pcur_pattern + pos, &pslppriv->slp_patterns[k].len, sizeof(pslppriv->slp_patterns[k].len));
		pos += sizeof(pslppriv->slp_patterns[k].len);

		/* 2: pattern content */
		_rtw_memcpy(pcur_pattern + pos, pslppriv->slp_patterns[k].content, pslppriv->slp_patterns[k].len);
		pos += pslppriv->slp_patterns[k].len;
	}
...
int rtw_wow_slp_get_pttern_len(int pattern_content_len)
{
        /* length field + content field */
	return (2 + pattern_content_len);
}

int rtw_wow_slp_check_pattern_len(_adapter *adapter)
{
	struct pwrctrl_priv *pwrpriv = adapter_to_pwrctl(adapter);	
	struct wow_slp_pattern_priv *pslppriv = &pwrpriv->slppriv;
	int index = 0, pattern_len = 0;
	...
	pattern_len = rtw_wow_slp_get_pttern_len(pslppriv->slp_patterns[index].len);

	/* RTW_INFO("%s: pattetn %d, all pattern %d, total patterns %d\n", __func__, 
		pslppriv->slp_patterns[index].len, pattern_len, (pslppriv->wowlan_slp_total_len + pattern_len)); */

	if( pslppriv->slp_patterns[index].len > MAX_SLP_PATTERN_SIZE)
	{
		RTW_PRINT("WARNING: the length of input pattern (%d) is greater than %d.\n", 
						pattern_len, MAX_SLP_PATTERN_SIZE);
		return PATTERN_ERROR_EXCEEDING_PATTERN_LEN; 
	}	
	else if((pslppriv->wowlan_slp_total_len + pattern_len) > MAX_TOTAL_SLP_PATTERN_SIZE)
	{
		RTW_PRINT("WARNING: the length of total patterns (%d) is greater than %d.\n", 
						(pslppriv->wowlan_slp_total_len + pattern_len), MAX_TOTAL_SLP_PATTERN_SIZE);
		return PATTERN_ERROR_EXCEEDING_TOTAL_PATTERN_LEN;
	}	
	else 
		return pattern_len;
...
void rtw_wow_war_parms_reset(_adapter *adapter)
{
    struct pwrctrl_priv *pwrpriv = adapter_to_pwrctl(adapter);
    u8 objid[] = {0x01, 0x02, 0x01, 0x02, 0x01, 0x4f, 0x02, 0x01};

    _rtw_memset(pwrpriv->wowlan_war_offload_snmp_community_name, 0, sizeof(pwrpriv->wowlan_war_offload_snmp_community_name));
    _rtw_memset(pwrpriv->wowlan_war_offload_snmp_display_string, 0, sizeof(pwrpriv->wowlan_war_offload_snmp_display_string));
    _rtw_memset(pwrpriv->wowlan_war_offload_snmp_object_id, 0, sizeof(pwrpriv->wowlan_war_offload_snmp_object_id));
    _rtw_memset(pwrpriv->wowlan_war_offload_snmp_object_name, 0, sizeof(pwrpriv->wowlan_war_offload_snmp_object_name));

    pwrpriv->wowlan_war_offload_snmp_community_name_len[0] = 0;
    pwrpriv->wowlan_war_offload_snmp_community_name_len[1] = 0;
    pwrpriv->wowlan_war_offload_snmp_device_status = 1; // unknown
    pwrpriv->wowlan_war_offload_snmp_printer_status = 2;    // unknown
    pwrpriv->wowlan_war_offload_snmp_sleep_status = 3;    // super sleep state
    pwrpriv->wowlan_war_offload_snmp_error_status = 0;  // no-error
    pwrpriv->wowlan_war_offload_snmp_display_string_len = 0;
    pwrpriv->wowlan_war_offload_snmp_enterprise_id = 18334; // km enterprise-id
    pwrpriv->wowlan_war_offload_snmp_object_id_len = 0;
    pwrpriv->wowlan_war_offload_snmp_object_name_len = 0;
	
#if defined(CONFIG_OFFLOAD_SSDP_V4) || defined(CONFIG_OFFLOAD_SSDP_V6)
	rtw_wow_ssdp_pattern_sw_reset(adapter);
#endif /* defined(CONFIG_OFFLOAD_SSDP_V4) || defined(CONFIG_OFFLOAD_SSDP_V6) */
#if defined(CONFIG_OFFLOAD_WSD_V4) || defined(CONFIG_OFFLOAD_WSD_V6)
            rtw_wow_wsd_pattern_sw_reset(adapter);
#endif /* defined(CONFIG_OFFLOAD_WSD_V4) || defined(CONFIG_OFFLOAD_WSD_V6) */
#if defined(CONFIG_OFFLOAD_SLP_V4) || defined(CONFIG_OFFLOAD_SLP_V6)
        rtw_wow_slp_pattern_sw_reset(adapter);
#endif /* defined(CONFIG_OFFLOAD_SLP_V4) || defined(CONFIG_OFFLOAD_SLP_V6) */

    // enterprise id
    pwrpriv->wowlan_war_offload_snmp_enterprise_id_buf[2] = 18334%128;
    pwrpriv->wowlan_war_offload_snmp_enterprise_id_buf[1] = ((18334 / 128)%128) | 0x80;
    pwrpriv->wowlan_war_offload_snmp_enterprise_id_buf[0] = ((18334 / 128) / 128) | 0x80;

    // community name
    snprintf(pwrpriv->wowlan_war_offload_snmp_community_name[0], sizeof(pwrpriv->wowlan_war_offload_snmp_community_name[0]), "public");
    pwrpriv->wowlan_war_offload_snmp_community_name_len[0] = strlen(pwrpriv->wowlan_war_offload_snmp_community_name[0]);

    // display string
    snprintf(pwrpriv->wowlan_war_offload_snmp_display_string, sizeof(pwrpriv->wowlan_war_offload_snmp_display_string), "generic");
    pwrpriv->wowlan_war_offload_snmp_display_string_len = strlen(pwrpriv->wowlan_war_offload_snmp_display_string);

    // object id
    _rtw_memcpy(pwrpriv->wowlan_war_offload_snmp_object_id, objid, sizeof(objid));
    pwrpriv->wowlan_war_offload_snmp_object_id_len = sizeof(objid);

    // object name
    snprintf(pwrpriv->wowlan_war_offload_snmp_object_name, sizeof(pwrpriv->wowlan_war_offload_snmp_object_name), "1234");
    pwrpriv->wowlan_war_offload_snmp_object_name_len = strlen(pwrpriv->wowlan_war_offload_snmp_object_name);
}

#ifdef CONFIG_OFFLOAD_NETBIOS

struct netbios_char_mapping ntb_char_mapping_table[60] =
{
    /* alphabet : 0 ~ 25 */
    {'A',"EB"}, {'B',"EC"}, {'C',"ED"}, {'D',"EE"}, {'E',"EF"},
    {'F',"EG"}, {'G',"EH"}, {'H',"EI"}, {'I',"EJ"}, {'J',"EK"},
    {'K',"EL"}, {'L',"EM"}, {'M',"EN"}, {'N',"EO"}, {'O',"EP"},
    {'P',"FA"}, {'Q',"FB"}, {'R',"FC"}, {'S',"FD"}, {'T',"FE"},
    {'U',"FF"}, {'V',"FG"}, {'W',"FH"}, {'X',"FI"}, {'Y',"FJ"},
    {'Z',"FK"},

    /* number : 26 ~ 35 */
    {'0',"DA"}, {'1',"DB"}, {'2',"DC"}, {'3',"DD"}, {'4',"DE"},
    {'5',"DF"}, {'6',"DG"}, {'7',"DH"}, {'8',"DI"}, {'9',"DJ"},
...
void rtw_netbios_set_param(_adapter *adapter)
{
    struct dvobj_priv *dvobj = adapter_to_dvobj(adapter);
    ...
    /* 1: set netbios name from user or system */
    if(0 == pwrpriv->wowlan_war_offload_netbios.ntbname_len)
    {
        /* RTW_INFO("==> hostname (str): %s(%zu)\n", utsname()->nodename, name_len); */
        _rtw_memset(pwrpriv->wowlan_war_offload_netbios.ntbname,0x00,MAX_NETBIOS_NAME_LEN);
        pwrpriv->wowlan_war_offload_netbios.ntbname_len = (name_len > MAX_NETBIOS_NAME_LEN)? MAX_NETBIOS_NAME_LEN: name_len;
        _rtw_memcpy(pwrpriv->wowlan_war_offload_netbios.ntbname, utsname()->nodename, pwrpriv->wowlan_war_offload_netbios.ntbname_len);
        pwrpriv->wowlan_war_offload_netbios.ntbname[pwrpriv->wowlan_war_offload_netbios.ntbname_len] = '\0';        
    }
    
    RTW_INFO("==> hostname'(str): [%s](%d)\n", pwrpriv->wowlan_war_offload_netbios.ntbname, pwrpriv->wowlan_war_offload_netbios.ntbname_len);

    /* 2: encode the netbios name */
    rtw_netbios_get_encoded_name(pwrpriv->wowlan_war_offload_netbios.ntbname, 
        pwrpriv->wowlan_war_offload_netbios.ntbname_len,
        pwrpriv->wowlan_war_offload_netbios.ntbname_encoded);

    /* 3: dump result */
#if 0    
    for(idx=0; idx<(MAX_NETBIOS_NAME_LEN*2); idx++)
        RTW_INFO("[%d]: %x\n", idx, pwrpriv->wowlan_war_offload_netbios.ntbname_encoded[idx]);
#endif

}


u8 rtw_netbios_get_encoded_name(u8 *str_in, u8 str_len, u8 *str_out)
{
    u8 idx, table_idx=60, i;

    for(idx=0; idx < 15 ; idx++)
    {
        if(idx < str_len )
        {
            if(str_in[idx] >= 'A' && str_in[idx] <= 'Z' ) /* upper alphabet */
                table_idx = str_in[idx] - 0x41 ;
            else if(str_in[idx] >= 'a' && str_in[idx] <= 'z' ) /* lower alphabet */
                table_idx = str_in[idx] - 0x61 ;
            else if(str_in[idx] >= '0' && str_in[idx] <= '9' ) /* number */
                table_idx = str_in[idx] - 0x30 + 26 ;
            else
            {   
                for( i=36 ; i<60; i++ )
                {
                    if( str_in[idx] == ntb_char_mapping_table[i].target_char )
                    {
                        table_idx = i; break;
                    }
                 }    
        ...
        {        
            table_idx = 36 ; /* append spaces if the length of str_in is less than 15 */
        }

        if(table_idx < 60)
        {
            str_out[idx*2]   = ntb_char_mapping_table[table_idx].target_ascii[0];
            str_out[idx*2+1] = ntb_char_mapping_table[table_idx].target_ascii[1];
            
            /* RTW_INFO("[%d]: %c(%d):%x %x\n", idx, str_in[idx], table_idx, str_out[idx*2], str_out[idx*2+1]); */
        }else
            return FALSE;
    }
...
void rtw_wow_wake_pattern_sw_reset(_adapter *adapter)
{
	int i;
	struct pwrctrl_priv *pwrctrlpriv = adapter_to_pwrctl(adapter);

	pwrctrlpriv->wpriv.wowlan_wake_pattern_idx = 0;
	pwrctrlpriv->wpriv.wowlan_wake_total_len = 0;

	for (i = 0 ; i < MAX_WAKE_NUM; i++) {
		_rtw_memset(pwrctrlpriv->wpriv.wake_patterns[i].content, '\0', MAX_WAKE_PATTERN_SIZE);
		_rtw_memset(pwrctrlpriv->wpriv.wake_patterns[i].mask, '\0', MAX_WAKE_SIZE);
		pwrctrlpriv->wpriv.wake_patterns[i].len = 0;
	}
}


bool rtw_wow_wake_pattern_parser_pattern_cmd(u8 *input, char *pattern,
				   u16 *pattern_len, char *bit_mask)
{
	...
	/* reset */
	_rtw_memset(pattern, '\0', MAX_WAKE_PATTERN_SIZE);
	_rtw_memset(bit_mask, '\0', MAX_WAKE_SIZE);
	(*pattern_len) = 0;
	...
	while (1) {
		...
		if (bit_mask && (strcmp(input, "-") == 0 ||
			...
			/* RTW_INFO("==> in; input-member = %s, hex = %x,  pattern[pos] = %x\n", member, hex, pattern[pos]); */
			...
			for(idx = 0; idx<2;idx++)
			{
				...
				if(member[idx] == 'x' || member[idx] == '-' || member[idx] == 'X')
				{
					member[idx] = '0' ;
					res = sscanf(member, "%02hhx", &hex);

					if(idx == 1)
						pattern[pos] = pattern[pos]<<4;	
					else
						pattern[pos] = hex & 0xff;
						
					
					/* RTW_INFO("%s: pattern is wildcard!!(%c), (%x)\n", __func__, member[idx], pattern[pos]); */
				}	
				else if(!IsHexDigit(member[idx]))
				{
					RTW_ERR("%s:[ERROR] pattern is invalid!!(%c)\n",__func__, member[idx]);
					goto error;
				}else if (bit_mask)
					bit_mask[mask_pos] |= 1 << (pos_in_unit_as_4bit % 8);
...
bool rtw_wow_wake_pattern_get_pattern_content(_adapter *adapter, u8 *pcur_pattern, u32 *PatternLength)
{
	struct pwrctrl_priv *pwrpriv = adapter_to_pwrctl(adapter);
	...
	_rtw_memset(pcur_pattern, 0, MAX_TOTAL_WAKE_PATTERN_SIZE);

	*PatternLength = pwpriv->wowlan_wake_total_len ;
	cur_pattern_num = pwpriv->wowlan_wake_pattern_idx;
	
	for (k = 0; k < cur_pattern_num ; k++) {

		/* 1: pattern length */
		_rtw_memcpy(pcur_pattern + pos, &pwpriv->wake_patterns[k].len, sizeof(pwpriv->wake_patterns[k].len));
		pos += sizeof(pwpriv->wake_patterns[k].len);

		/* 2: pattern content */
		_rtw_memcpy(pcur_pattern + pos, pwpriv->wake_patterns[k].content, pwpriv->wake_patterns[k].len);
		pos += pwpriv->wake_patterns[k].len;

		/* 3: pattern bitmask */
		len_mask = ( pwpriv->wake_patterns[k].len/4 ) +  (( pwpriv->wake_patterns[k].len%4==0)?0:1);
		_rtw_memcpy(pcur_pattern + pos, pwpriv->wake_patterns[k].mask, len_mask);
		pos += len_mask;
	}
...
int rtw_wow_wake_pattern_get_pttern_len(int pattern_content_len)
{
	int len_mask = ( pattern_content_len/4 ) + (( pattern_content_len%4==0)?0:1);
	return (2 + pattern_content_len + len_mask);
}

int rtw_wow_wake_pattern_check_pattern_len(_adapter *adapter)
{
	struct pwrctrl_priv *pwrpriv = adapter_to_pwrctl(adapter);	
	struct wow_wake_pattern_priv *pwpriv = &pwrpriv->wpriv;
	int index = 0, pattern_len = 0;
	...
	pattern_len = rtw_wow_wake_pattern_get_pttern_len(pwpriv->wake_patterns[index].len);

	/* RTW_INFO("%s: pattetn %d, all pattern %d, total patterns %d\n", __func__, 
		pwpriv->wake_patterns[index].len, pattern_len, (pwpriv->wowlan_wake_total_len + pattern_len)); */

	if( pwpriv->wake_patterns[index].len > MAX_WAKE_PATTERN_SIZE)
	{
		RTW_PRINT("WARNING: the length of input pattern (%d) is greater than %d.\n", 
						pattern_len, MAX_WAKE_PATTERN_SIZE);
		return PATTERN_ERROR_EXCEEDING_PATTERN_LEN; 
	}	
	else if((pwpriv->wowlan_wake_total_len + pattern_len) > MAX_TOTAL_WAKE_PATTERN_SIZE)
	{
		RTW_PRINT("WARNING: the length of total patterns (%d) is greater than %d.\n", 
						(pwpriv->wowlan_wake_total_len + pattern_len), MAX_TOTAL_WAKE_PATTERN_SIZE);
		return PATTERN_ERROR_EXCEEDING_TOTAL_PATTERN_LEN;
	}	
	else 
		return pattern_len;
