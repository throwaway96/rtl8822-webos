...
#ifdef CONFIG_OFFLOAD_MAGIC_PKT
	struct registry_priv  *registry_par = &adapter->registrypriv;
#endif /* CONFIG_OFFLOAD_MAGIC_PKT */
...
#ifdef CONFIG_OFFLOAD_MAGIC_PKT
		if (WAR_MAGIC_WAKEUP_EN & pwrpriv->wowlan_war_offload_ctrl) {
			RTW_PRINT_SEL(m, "\n Magic Packet wakeup enabled, (original wakeup event: %#02x)\n", registry_par->wakeup_event);
		}
#endif /* CONFIG_OFFLOAD_MAGIC_PKT */
...
#ifdef CONFIG_OFFLOAD_NETBIOS
            /* init. netbios name and other parameters for netbios offload */
            if ((WAR_NETBIOS_RSP_EN & pwrpriv->wowlan_war_offload_ctrl)&&(pwrpriv->wowlan_war_offload_netbios.ntbname_len==0))
                    rtw_netbios_set_param(padapter);
#endif /* CONFIG_OFFLOAD_NETBIOS */
#if defined(CONFIG_OFFLOAD_LLMNR_V4) ||defined(CONFIG_OFFLOAD_LLMNR_V6)  
	/* init. llmnr name */
	if ( ( (WAR_LLMNR_V4_RSP_EN & pwrpriv->wowlan_war_offload_ctrl) ||
		(WAR_LLMNR_V6_RSP_EN & pwrpriv->wowlan_war_offload_ctrl)) &&
		(pwrpriv->wowlan_war_offload_llmnr_data_len == 0)){
		
		 long int name_len = strlen(utsname()->nodename);
		_rtw_memset(pwrpriv->wowlan_war_offload_llmnr_data, 0x00, MAX_LLMNR_DATA_LEN);
		pwrpriv->wowlan_war_offload_llmnr_data_len = name_len;
		_rtw_memcpy(pwrpriv->wowlan_war_offload_llmnr_data, utsname()->nodename, pwrpriv->wowlan_war_offload_llmnr_data_len);
		pwrpriv->wowlan_war_offload_llmnr_data[pwrpriv->wowlan_war_offload_llmnr_data_len] = '\0';       
	}
		
#endif /* defined(CONFIG_OFFLOAD_LLMNR_V4) ||defined(CONFIG_OFFLOAD_LLMNR_V6)  */
...
int proc_get_offload_ipv4_addr(struct seq_file *m, void *v)
{
	struct net_device *dev = m->private;
	_adapter *adapter = (_adapter *)rtw_netdev_priv(dev);
	...
	if (_TRUE == pwrpriv->wowlan_war_offload_mode) {
#if defined(CONFIG_OFFLOAD_ICMP_V4) || defined(CONFIG_OFFLOAD_NETBIOS)
		struct war_ipv4_fmt* pip_info = &pwrpriv->wowlan_war_offload_ipv4;
		int i = 0;

		for (i=0;i<4;i++) {
			if (pip_info->ip_addr[i] == 0) {
				if (i == 0) {
					_rtw_memcpy(&pip_info->ip_addr[i], pmlmeinfo->ip_addr, 4);
				} else {
					continue;
				}
			} 
			RTW_PRINT_SEL(m, "\nIP(%d) Address=%d.%d.%d.%d ", (i+1), (pip_info->ip_addr[i] & 0x000000ff), (pip_info->ip_addr[i] & 0x0000ff00)>>8, (pip_info->ip_addr[i] & 0x00ff0000)>>16, (pip_info->ip_addr[i] & 0xff000000)>>24);
			RTW_PRINT_SEL(m, "Subnet=%d.%d.%d.%d ", (pip_info->ip_subnet[i] & 0x000000ff), (pip_info->ip_subnet[i] & 0x0000ff00)>>8, (pip_info->ip_subnet[i] & 0x00ff0000)>>16, (pip_info->ip_subnet[i] & 0xff000000)>>24);
			RTW_PRINT_SEL(m, "Gateway=%d.%d.%d.%d\n", (pip_info->ip_gateway[i] & 0x000000ff), (pip_info->ip_gateway[i] & 0x0000ff00)>>8, (pip_info->ip_gateway[i] & 0x00ff0000)>>16, (pip_info->ip_gateway[i] & 0xff000000)>>24);
		}
#endif /* CONFIG_OFFLOAD_ICMP_V4 || CONFIG_OFFLOAD_NETBIOS */
	}

	return 0;
}

ssize_t proc_set_offload_ipv4_addr(struct file *file, const char __user *buffer, size_t count, loff_t *pos, void *data)
{
	struct net_device *dev = data;
	_adapter *padapter = (_adapter *)rtw_netdev_priv(dev);
	struct dvobj_priv *dvobj = adapter_to_dvobj(padapter);
...
int proc_get_offload_ipv6_addr(struct seq_file *m, void *v)
{
	struct net_device *dev = m->private;
	_adapter *adapter = (_adapter *)rtw_netdev_priv(dev);
	struct mlme_ext_priv	*pmlmeext = &adapter->mlmeextpriv;
	...
	if (_TRUE == pwrpriv->wowlan_war_offload_mode) {
#if defined(CONFIG_OFFLOAD_ICMP_V6) || defined(CONFIG_OFFLOAD_ICMP_NS)
		struct war_ipv6_fmt* pip_info = &pwrpriv->wowlan_war_offload_ipv6;
		u8 zero_ary[16] = {0x00};
		int i = 0;

		for (i=0;i<8;i++) {
			if (_rtw_memcmp(pip_info->ipv6_addr[i], zero_ary, RTW_IPv6_ADDR_LEN) == _TRUE) {
				if (i == 0) {
					_rtw_memcpy(&pip_info->ipv6_addr[i], pmlmeinfo->ip6_addr, RTW_IPv6_ADDR_LEN);
				} else {
					continue;
				}
			}
			RTW_PRINT_SEL(m, "\nIPv6(%d) Address=%x:%x:%x:%x:%x:%x:%x:%x\n", (i+1), RTW_GET_BE16(pip_info->ipv6_addr[i]), RTW_GET_BE16(pip_info->ipv6_addr[i]+2), RTW_GET_BE16(pip_info->ipv6_addr[i]+4), RTW_GET_BE16(pip_info->ipv6_addr[i]+6), RTW_GET_BE16(pip_info->ipv6_addr[i]+8), RTW_GET_BE16(pip_info->ipv6_addr[i]+10), RTW_GET_BE16(pip_info->ipv6_addr[i]+12), RTW_GET_BE16(pip_info->ipv6_addr[i]+14));
		}
#endif /* CONFIG_OFFLOAD_ICMP_V6 || CONFIG_OFFLOAD_ICMP_NS */
	}
	return 0;
}

ssize_t proc_set_offload_ipv6_addr(struct file *file, const char __user *buffer, size_t count, loff_t *pos, void *data)
{
	struct net_device *dev = data;
	_adapter *padapter = (_adapter *)rtw_netdev_priv(dev);
	struct dvobj_priv *dvobj = adapter_to_dvobj(padapter);
...
int proc_get_offload_mac_addr(struct seq_file *m, void *v)
{
	struct net_device *dev = m->private;
	_adapter *adapter = (_adapter *)rtw_netdev_priv(dev);
	struct mlme_ext_priv	*pmlmeext = &adapter->mlmeextpriv;
	...
	if (_TRUE == pwrpriv->wowlan_war_offload_mode) {
		if(is_zero_mac_addr(pwrpriv->wowlan_war_offload_mac)) {
			_rtw_memcpy(pwrpriv->wowlan_war_offload_mac, adapter_mac_addr(adapter), 6);
		}
		RTW_PRINT_SEL(m, "\n Mac Address:%02x:%02x:%02x:%02x:%02x:%02x\n", pwrpriv->wowlan_war_offload_mac[0], pwrpriv->wowlan_war_offload_mac[1], pwrpriv->wowlan_war_offload_mac[2], pwrpriv->wowlan_war_offload_mac[3], pwrpriv->wowlan_war_offload_mac[4], pwrpriv->wowlan_war_offload_mac[5]);
	} else {
		RTW_PRINT_SEL(m, "\n[ Offload Feature Disabled ]\n");
	}
	return 0;
...
ssize_t proc_set_offload_mac_addr(struct file *file, const char __user *buffer, size_t count, loff_t *pos, void *data)
{
	struct net_device *dev = data;
	_adapter *padapter = (_adapter *)rtw_netdev_priv(dev);
	struct dvobj_priv *dvobj = adapter_to_dvobj(padapter);
	struct pwrctrl_priv *pwrpriv = dvobj_to_pwrctl(dvobj);
	char tmp[32];
	u8 addr_0 = 0, addr_1 = 0, addr_2 = 0, addr_3 = 0, addr_4 = 0, addr_5 = 0;
	...
	if (buffer && !copy_from_user(tmp, buffer, count)) {
		int num = sscanf(tmp, "%hhx:%hhx:%hhx:%hhx:%hhx:%hhx", &addr_0, &addr_1, &addr_2, &addr_3, &addr_4, &addr_5);
		if (num == 6) {
			pwrpriv->wowlan_war_offload_mac[0] = addr_0;
			pwrpriv->wowlan_war_offload_mac[1] = addr_1;
			pwrpriv->wowlan_war_offload_mac[2] = addr_2;
			pwrpriv->wowlan_war_offload_mac[3] = addr_3;
			pwrpriv->wowlan_war_offload_mac[4] = addr_4;
			pwrpriv->wowlan_war_offload_mac[5] = addr_5;
			RTW_INFO("Store war_offload_mac address %x:%x:%x:%x:%x:%x\n", pwrpriv->wowlan_war_offload_mac[0], pwrpriv->wowlan_war_offload_mac[1], pwrpriv->wowlan_war_offload_mac[2], pwrpriv->wowlan_war_offload_mac[3], pwrpriv->wowlan_war_offload_mac[4], pwrpriv->wowlan_war_offload_mac[5]);
		}
	}
	return count;
}
...
#ifdef CONFIG_OFFLOAD_NETBIOS
int proc_get_offload_netbios_name(struct seq_file *m, void *v)
{    
	struct net_device *dev = m->private;
	_adapter *adapter = (_adapter *)rtw_netdev_priv(dev);
	struct dvobj_priv *dvobj = adapter_to_dvobj(adapter);
	struct pwrctrl_priv *pwrpriv = dvobj_to_pwrctl(dvobj);
    struct war_netbios_fmt *pntb_para = &pwrpriv->wowlan_war_offload_netbios;
    u8 i=0;


    if (WAR_NETBIOS_RSP_EN & pwrpriv->wowlan_war_offload_ctrl)
    {
        RTW_PRINT_SEL(m, "\n NetBIOS Name = %s(%d)\n", pntb_para->ntbname, pntb_para->ntbname_len);
        RTW_PRINT_SEL(m, "\n NetBIOS Encoded Name = ");
        for(i=0; i< (MAX_NETBIOS_NAME_LEN*2) ; i++)
            RTW_PRINT_SEL(m, "%x ", pntb_para->ntbname_encoded[i]);
        RTW_PRINT_SEL(m, "\n");
    }else
        RTW_PRINT_SEL(m, "\n Please enable offload NetBIOS first.\n");

    return 0;
}
ssize_t proc_set_offload_netbios_name(struct file *file, const char __user *buffer, size_t count, loff_t *pos, void *data)
{
	struct net_device *dev = data;
	_adapter *padapter = (_adapter *)rtw_netdev_priv(dev);
	struct dvobj_priv *dvobj = adapter_to_dvobj(padapter);
	struct pwrctrl_priv *pwrpriv = dvobj_to_pwrctl(dvobj);
    	struct war_netbios_fmt *pntb_para = &pwrpriv->wowlan_war_offload_netbios;
	char tmp[MAX_NETBIOS_NAME_LEN+1], tmp_name[MAX_NETBIOS_NAME_LEN+1]; // it's a string, need one more char for '\0'
	...
	if (count > sizeof(tmp)) {
		RTW_INFO(FUNC_ADPT_FMT ": input length is too large (%zu > max len 15)\n", FUNC_ADPT_ARG(padapter), count);
		rtw_warn_on(1);
		return -EFAULT;
	}

	if (buffer && !copy_from_user(tmp, buffer, count)) {
		int num = sscanf(tmp, "%s", tmp_name);
        
        RTW_INFO("==> user set NetBios name : [%s] \n", tmp_name);
		if (num == 1) {

            if(_rtw_memcmp(tmp_name,"reset",5))
                pntb_para->ntbname_len = 0; /* reset netbios name from system */
            else
            {
                pntb_para->ntbname_len = strlen(tmp_name);
                _rtw_memset(pntb_para->ntbname,0x00,MAX_NETBIOS_NAME_LEN);
                _rtw_memcpy(pntb_para->ntbname,tmp_name, pntb_para->ntbname_len);
            }
		}
	}
...
int proc_get_offload_llmnr_name(struct seq_file *m, void *v)
{    
	struct net_device *dev = m->private;
	_adapter *adapter = (_adapter *)rtw_netdev_priv(dev);
	struct dvobj_priv *dvobj = adapter_to_dvobj(adapter);
	...
	{
		RTW_PRINT_SEL(m, "\n LLMNR Data = %s(%d)\n", 
			pwrpriv->wowlan_war_offload_llmnr_data,
			pwrpriv->wowlan_war_offload_llmnr_data_len);
	}else{
		RTW_PRINT_SEL(m, "\n Current configuration does not support  LLMNR.\n");
	}	

	return 0;
}
ssize_t proc_set_offload_llmnr_name(struct file *file, const char __user *buffer, size_t count, loff_t *pos, void *data)
{
	struct net_device *dev = data;
	_adapter *padapter = (_adapter *)rtw_netdev_priv(dev);
	struct dvobj_priv *dvobj = adapter_to_dvobj(padapter);
	struct pwrctrl_priv *pwrpriv = dvobj_to_pwrctl(dvobj);
    	struct war_netbios_fmt *pntb_para = &pwrpriv->wowlan_war_offload_netbios;
	char tmp[MAX_LLMNR_DATA_LEN+1], tmp_data[MAX_LLMNR_DATA_LEN+1]; // it's a string, need one more char for '\0'
	...
	if (count > sizeof(tmp)) {
		RTW_INFO(FUNC_ADPT_FMT ": input length is too large (%zu > MAX_LLMNR_DATA_LEN %d)\n", FUNC_ADPT_ARG(padapter), count, MAX_LLMNR_DATA_LEN);
		rtw_warn_on(1);
		return -EFAULT;
	}
	...
	if ( ! ((WAR_LLMNR_V4_RSP_EN & pwrpriv->wowlan_war_offload_ctrl) || 
		(WAR_LLMNR_V6_RSP_EN & pwrpriv->wowlan_war_offload_ctrl))  ) {
		RTW_INFO(FUNC_ADPT_FMT ":Current configuration does not support  LLMNR.\n", FUNC_ADPT_ARG(padapter));
		return -EFAULT;
	}

	if (buffer && !copy_from_user(tmp, buffer, count)) {
		
		int num = sscanf(tmp, "%s", tmp_data);        
		RTW_INFO("==> Set LLMNR data : [%s] \n", tmp_data);
		
		if (num == 1) {
			if(_rtw_memcmp(tmp_data,"clean",5)){
				pwrpriv->wowlan_war_offload_llmnr_data_len = 0; 
				_rtw_memset(pwrpriv->wowlan_war_offload_llmnr_data, 0x00,MAX_LLMNR_DATA_LEN);
			}else{
				pwrpriv->wowlan_war_offload_llmnr_data_len = (strlen(tmp_data) < MAX_LLMNR_DATA_LEN)? strlen(tmp_data): MAX_LLMNR_DATA_LEN;
				_rtw_memset(pwrpriv->wowlan_war_offload_llmnr_data, 0x00,MAX_LLMNR_DATA_LEN);
				_rtw_memcpy(pwrpriv->wowlan_war_offload_llmnr_data, 
					tmp_data, pwrpriv->wowlan_war_offload_llmnr_data_len);
			}
		}
	}
...
#if defined(CONFIG_OFFLOAD_SNMP_V4) || defined(CONFIG_OFFLOAD_SNMP_V6)

int proc_get_offload_snmp_community_name(struct seq_file *m, void *v)
{
	struct net_device *dev = m->private;
	_adapter *adapter = (_adapter *)rtw_netdev_priv(dev);
	struct dvobj_priv *dvobj = adapter_to_dvobj(adapter);
	...
	if (_TRUE == pwrpriv->wowlan_war_offload_mode) {
	    if ((WAR_SNMP_V4_RSP_EN & pwrpriv->wowlan_war_offload_ctrl) ||
                (WAR_SNMP_V6_RSP_EN & pwrpriv->wowlan_war_offload_ctrl) ||
                (WAR_SNMP_V4_WAKEUP_EN & pwrpriv->wowlan_war_offload_ctrl) ||
                (WAR_SNMP_V6_WAKEUP_EN & pwrpriv->wowlan_war_offload_ctrl)) {
            for(i=0; i<MAX_SNMP_COMMUNITY_NAME_NUM;i++)
                if(pwrpriv->wowlan_war_offload_snmp_community_name_len[i])
        		    RTW_PRINT_SEL(m, "\nCommunity Name[%d]:%s length(%d)\n", i,
                        pwrpriv->wowlan_war_offload_snmp_community_name[i], 
                        pwrpriv->wowlan_war_offload_snmp_community_name_len[i]);
                else
                    RTW_PRINT_SEL(m, "\nCommunity Name[%d]: NULL\n",i );
        } else {
	        RTW_PRINT_SEL(m, "\nSNMP Not enabled\n");
        }
	} else {
...
ssize_t proc_set_offload_snmp_community_name(struct file *file, const char __user *buffer, size_t count, loff_t *pos, void *data)
{
	struct net_device *dev = data;
	_adapter *padapter = (_adapter *)rtw_netdev_priv(dev);
	struct dvobj_priv *dvobj = adapter_to_dvobj(padapter);
	...
	char tmp[32];
    int index = 0;
    char community_name[MAX_SNMP_COMMUNITY_NAME_LEN];
	...
	if (buffer && !copy_from_user(tmp, buffer, count)) {
        _rtw_memset(community_name, 0, MAX_SNMP_COMMUNITY_NAME_LEN);
		sscanf(tmp, "%d %s", &index, community_name);

        if (index >= MAX_SNMP_COMMUNITY_NAME_NUM) {
    		RTW_INFO(FUNC_ADPT_FMT ": index is over the limit (MAX_SNMP_COMMUNITY_NAME_NUM = %d)!\n", 
                FUNC_ADPT_ARG(padapter), MAX_SNMP_COMMUNITY_NAME_NUM);
    		return -EFAULT;
        }
        
        _rtw_memset(pwrpriv->wowlan_war_offload_snmp_community_name[index], 0, MAX_SNMP_COMMUNITY_NAME_LEN);
        _rtw_memcpy(pwrpriv->wowlan_war_offload_snmp_community_name[index], community_name, sizeof(community_name));
        pwrpriv->wowlan_war_offload_snmp_community_name_len[index] = (strlen(community_name) > MAX_SNMP_COMMUNITY_NAME_LEN)?MAX_SNMP_COMMUNITY_NAME_LEN:strlen(community_name);
        if(MAX_SNMP_COMMUNITY_NAME_LEN > pwrpriv->wowlan_war_offload_snmp_community_name_len[index]) {
            RTW_INFO("SNMP community name = %s\n", pwrpriv->wowlan_war_offload_snmp_community_name[index]);
        } else {
            RTW_INFO("SNMP community name is too long to print\n");
        }
    }
...
int proc_get_offload_snmp_device_status(struct seq_file *m, void *v)
{
	struct net_device *dev = m->private;
	_adapter *adapter = (_adapter *)rtw_netdev_priv(dev);
	struct dvobj_priv *dvobj = adapter_to_dvobj(adapter);
	...
	if (_TRUE == pwrpriv->wowlan_war_offload_mode) {
	    if ((WAR_SNMP_V4_RSP_EN & pwrpriv->wowlan_war_offload_ctrl) ||
                ...
                (WAR_SNMP_V4_WAKEUP_EN & pwrpriv->wowlan_war_offload_ctrl) ||
                (WAR_SNMP_V6_WAKEUP_EN & pwrpriv->wowlan_war_offload_ctrl)) {
		    RTW_PRINT_SEL(m, "\nDevice Status:%d\n", pwrpriv->wowlan_war_offload_snmp_device_status);
        } else {
	        RTW_PRINT_SEL(m, "\nSNMP Not enabled\n");
        }
	} else {
...
ssize_t proc_set_offload_snmp_device_status(struct file *file, const char __user *buffer, size_t count, loff_t *pos, void *data)
{
	struct net_device *dev = data;
	_adapter *padapter = (_adapter *)rtw_netdev_priv(dev);
	struct dvobj_priv *dvobj = adapter_to_dvobj(padapter);
	...
	if (buffer && !copy_from_user(tmp, buffer, count)) {
        int num = sscanf(tmp, "%d", &dev_status);
        if(1 == num) {
            pwrpriv->wowlan_war_offload_snmp_device_status = dev_status;
        }
    }
...
int proc_get_offload_snmp_printer_status(struct seq_file *m, void *v)
{
	struct net_device *dev = m->private;
	_adapter *adapter = (_adapter *)rtw_netdev_priv(dev);
	struct dvobj_priv *dvobj = adapter_to_dvobj(adapter);
	...
	if (_TRUE == pwrpriv->wowlan_war_offload_mode) {
	    if ((WAR_SNMP_V4_RSP_EN & pwrpriv->wowlan_war_offload_ctrl) ||
                ...
                (WAR_SNMP_V4_WAKEUP_EN & pwrpriv->wowlan_war_offload_ctrl) ||
                (WAR_SNMP_V6_WAKEUP_EN & pwrpriv->wowlan_war_offload_ctrl)) {
		    RTW_PRINT_SEL(m, "\nPrinter Status:%d\n", pwrpriv->wowlan_war_offload_snmp_printer_status);
        } else {
	        RTW_PRINT_SEL(m, "\nSNMP Not enabled\n");
        }
	} else {
...
ssize_t proc_set_offload_snmp_printer_status(struct file *file, const char __user *buffer, size_t count, loff_t *pos, void *data)
{
	struct net_device *dev = data;
	_adapter *padapter = (_adapter *)rtw_netdev_priv(dev);
	struct dvobj_priv *dvobj = adapter_to_dvobj(padapter);
	...
	if (buffer && !copy_from_user(tmp, buffer, count)) {
        int num = sscanf(tmp, "%d", &printer_status);
        if(1 == num) {
            pwrpriv->wowlan_war_offload_snmp_printer_status = printer_status;
        }
    }
...
int proc_get_offload_snmp_error_status(struct seq_file *m, void *v)
{
	struct net_device *dev = m->private;
	_adapter *adapter = (_adapter *)rtw_netdev_priv(dev);
	struct dvobj_priv *dvobj = adapter_to_dvobj(adapter);
	...
	if (_TRUE == pwrpriv->wowlan_war_offload_mode) {
	    if ((WAR_SNMP_V4_RSP_EN & pwrpriv->wowlan_war_offload_ctrl) ||
                ...
                (WAR_SNMP_V4_WAKEUP_EN & pwrpriv->wowlan_war_offload_ctrl) ||
                (WAR_SNMP_V6_WAKEUP_EN & pwrpriv->wowlan_war_offload_ctrl)) {
		    RTW_PRINT_SEL(m, "\nError Status:%02x\n", pwrpriv->wowlan_war_offload_snmp_error_status);
        } else {
	        RTW_PRINT_SEL(m, "\nSNMP Not enabled\n");
        }
	} else {
...
ssize_t proc_set_offload_snmp_error_status(struct file *file, const char __user *buffer, size_t count, loff_t *pos, void *data)
{
	struct net_device *dev = data;
	_adapter *padapter = (_adapter *)rtw_netdev_priv(dev);
	struct dvobj_priv *dvobj = adapter_to_dvobj(padapter);
	...
	if (buffer && !copy_from_user(tmp, buffer, count)) {
        int num = sscanf(tmp, "%x", &err_status);
        if(1 == num) {
            pwrpriv->wowlan_war_offload_snmp_error_status = err_status;
        }
    }
...
int proc_get_offload_snmp_display_string(struct seq_file *m, void *v)
{
	struct net_device *dev = m->private;
	_adapter *adapter = (_adapter *)rtw_netdev_priv(dev);
	struct dvobj_priv *dvobj = adapter_to_dvobj(adapter);
	...
	if (_TRUE == pwrpriv->wowlan_war_offload_mode) {
	    if ((WAR_SNMP_V4_RSP_EN & pwrpriv->wowlan_war_offload_ctrl) ||
                ...
                (WAR_SNMP_V4_WAKEUP_EN & pwrpriv->wowlan_war_offload_ctrl) ||
                (WAR_SNMP_V6_WAKEUP_EN & pwrpriv->wowlan_war_offload_ctrl)) {
		    RTW_PRINT_SEL(m, "\nDisplay String:%s length(%d)\n", pwrpriv->wowlan_war_offload_snmp_display_string, pwrpriv->wowlan_war_offload_snmp_display_string_len);
        } else {
	        RTW_PRINT_SEL(m, "\nSNMP Not enabled\n");
        }
	} else {
...
ssize_t proc_set_offload_snmp_display_string(struct file *file, const char __user *buffer, size_t count, loff_t *pos, void *data)
{
	struct net_device *dev = data;
	_adapter *padapter = (_adapter *)rtw_netdev_priv(dev);
	struct dvobj_priv *dvobj = adapter_to_dvobj(padapter);
	struct pwrctrl_priv *pwrpriv = dvobj_to_pwrctl(dvobj);
	char tmp[128];
    char display_string[MAX_SNMP_DISPLAY_STRING_LEN];
	...
	if (buffer && !copy_from_user(tmp, buffer, count)) {
        _rtw_memset(display_string, 0, MAX_SNMP_DISPLAY_STRING_LEN);
		sscanf(tmp, "%s", display_string);
        _rtw_memset(pwrpriv->wowlan_war_offload_snmp_display_string, 0, MAX_SNMP_DISPLAY_STRING_LEN);
        _rtw_memcpy(pwrpriv->wowlan_war_offload_snmp_display_string, display_string, sizeof(display_string));
        pwrpriv->wowlan_war_offload_snmp_display_string_len = (strlen(display_string) > MAX_SNMP_DISPLAY_STRING_LEN)?MAX_SNMP_DISPLAY_STRING_LEN:strlen(display_string);
        if(MAX_SNMP_DISPLAY_STRING_LEN > pwrpriv->wowlan_war_offload_snmp_display_string_len) {
            RTW_INFO("SNMP display string = %s\n", pwrpriv->wowlan_war_offload_snmp_display_string);
        } else {
            RTW_INFO("SNMP community name is too long to print\n");
        }
    }
...
int proc_get_offload_snmp_enterprise_id(struct seq_file *m, void *v)
{
	struct net_device *dev = m->private;
	_adapter *adapter = (_adapter *)rtw_netdev_priv(dev);
	struct dvobj_priv *dvobj = adapter_to_dvobj(adapter);
	...
	if (_TRUE == pwrpriv->wowlan_war_offload_mode) {
	    if ((WAR_SNMP_V4_RSP_EN & pwrpriv->wowlan_war_offload_ctrl) ||
                ...
                (WAR_SNMP_V4_WAKEUP_EN & pwrpriv->wowlan_war_offload_ctrl) ||
                (WAR_SNMP_V6_WAKEUP_EN & pwrpriv->wowlan_war_offload_ctrl)) {
		    RTW_PRINT_SEL(m, "\nEnterprise Id:%d\n", pwrpriv->wowlan_war_offload_snmp_enterprise_id);
        } else {
	        RTW_PRINT_SEL(m, "\nSNMP Not enabled\n");
        }
	} else {
...
ssize_t proc_set_offload_snmp_enterprise_id(struct file *file, const char __user *buffer, size_t count, loff_t *pos, void *data)
{
	struct net_device *dev = data;
	_adapter *padapter = (_adapter *)rtw_netdev_priv(dev);
	struct dvobj_priv *dvobj = adapter_to_dvobj(padapter);
	...
	if (buffer && !copy_from_user(tmp, buffer, count)) {
        int num = sscanf(tmp, "%u", &enterprise_id);
        if(1 == num) {
            pwrpriv->wowlan_war_offload_snmp_enterprise_id = enterprise_id;
            pwrpriv->wowlan_war_offload_snmp_enterprise_id_buf[2] = (enterprise_id%128);
            pwrpriv->wowlan_war_offload_snmp_enterprise_id_buf[1] = (((enterprise_id/128)%128) | 0x80);	
            pwrpriv->wowlan_war_offload_snmp_enterprise_id_buf[0] = (((enterprise_id/128)/128) | 0x80);	
            RTW_INFO("Enterprise ID = %02x-%02x-%02x\n", pwrpriv->wowlan_war_offload_snmp_enterprise_id_buf[0],
			pwrpriv->wowlan_war_offload_snmp_enterprise_id_buf[1], pwrpriv->wowlan_war_offload_snmp_enterprise_id_buf[2]);
        }
    }
...
int proc_get_offload_snmp_object_id(struct seq_file *m, void *v)
{
	struct net_device *dev = m->private;
	_adapter *adapter = (_adapter *)rtw_netdev_priv(dev);
	struct dvobj_priv *dvobj = adapter_to_dvobj(adapter);
	...
	if (_TRUE == pwrpriv->wowlan_war_offload_mode) {
	    if ((WAR_SNMP_V4_RSP_EN & pwrpriv->wowlan_war_offload_ctrl) ||
                ...
                (WAR_SNMP_V6_WAKEUP_EN & pwrpriv->wowlan_war_offload_ctrl)) {
		    RTW_PRINT_SEL(m, "\nObject Id:");
            for (i=0;i<pwrpriv->wowlan_war_offload_snmp_object_id_len;i++) {
		        RTW_PRINT_SEL(m, " %02x", pwrpriv->wowlan_war_offload_snmp_object_id[i]);
            }
		    RTW_PRINT_SEL(m, " with length(%d)\n", pwrpriv->wowlan_war_offload_snmp_object_id_len);
        } else {
	        RTW_PRINT_SEL(m, "\nSNMP Not enabled\n");
        }
	} else {
...
ssize_t proc_set_offload_snmp_object_id(struct file *file, const char __user *buffer, size_t count, loff_t *pos, void *data)
{
	struct net_device *dev = data;
	_adapter *padapter = (_adapter *)rtw_netdev_priv(dev);
	struct dvobj_priv *dvobj = adapter_to_dvobj(padapter);
	...
	if (buffer && !copy_from_user(tmp, buffer, count)) {
        _rtw_memset(object_id, 0, sizeof(object_id));
		sscanf(tmp, "%s", object_id);
        for (i=0;i<strlen(object_id);i+=2) {
			char c0,c1;
			c0 = is_alpha(alpha_to_upper(object_id[i]))?(alpha_to_upper(object_id[i])-'A'+10):(object_id[i]-'0');
			c1 = is_alpha(alpha_to_upper(object_id[i+1]))?(alpha_to_upper(object_id[i+1])-'A'+10):(object_id[i+1]-'0');
			
            pwrpriv->wowlan_war_offload_snmp_object_id[i/2] = c0*16 + c1;
        }
        pwrpriv->wowlan_war_offload_snmp_object_id_len = ((strlen(object_id)/2) > MAX_SNMP_OBJECT_ID_LEN)?MAX_SNMP_OBJECT_ID_LEN:(strlen(object_id)/2);
        if(MAX_SNMP_OBJECT_ID_LEN > pwrpriv->wowlan_war_offload_snmp_object_id_len) {
            RTW_INFO("SNMP object id = ");
            for (i=0;i<pwrpriv->wowlan_war_offload_snmp_object_id_len;i++) {
                RTW_INFO("%d", pwrpriv->wowlan_war_offload_snmp_object_id[i]);
                if(i<pwrpriv->wowlan_war_offload_snmp_object_id_len-1)
                    RTW_INFO(".");
            }
            RTW_INFO("\n");
        } else {
            RTW_INFO("SNMP object id is too long to print\n");
        }
    }
...
int proc_get_offload_snmp_object_name(struct seq_file *m, void *v)
{
	struct net_device *dev = m->private;
	_adapter *adapter = (_adapter *)rtw_netdev_priv(dev);
	struct dvobj_priv *dvobj = adapter_to_dvobj(adapter);
	...
	if (_TRUE == pwrpriv->wowlan_war_offload_mode) {
	    if ((WAR_SNMP_V4_RSP_EN & pwrpriv->wowlan_war_offload_ctrl) ||
                ...
                (WAR_SNMP_V4_WAKEUP_EN & pwrpriv->wowlan_war_offload_ctrl) ||
                (WAR_SNMP_V6_WAKEUP_EN & pwrpriv->wowlan_war_offload_ctrl)) {
		    RTW_PRINT_SEL(m, "\nObject Name:%s length(%d)\n", pwrpriv->wowlan_war_offload_snmp_object_name, pwrpriv->wowlan_war_offload_snmp_object_name_len);
        } else {
	        RTW_PRINT_SEL(m, "\nSNMP Not enabled\n");
        }
	} else {
...
ssize_t proc_set_offload_snmp_object_name(struct file *file, const char __user *buffer, size_t count, loff_t *pos, void *data)
{
	struct net_device *dev = data;
	_adapter *padapter = (_adapter *)rtw_netdev_priv(dev);
	struct dvobj_priv *dvobj = adapter_to_dvobj(padapter);
	struct pwrctrl_priv *pwrpriv = dvobj_to_pwrctl(dvobj);
	char tmp[32];
    char object_name[MAX_SNMP_OBJECT_NAME_LEN];
	...
	if (buffer && !copy_from_user(tmp, buffer, count)) {
        _rtw_memset(object_name, 0, MAX_SNMP_OBJECT_NAME_LEN);
		sscanf(tmp, "%s", object_name);
        _rtw_memset(pwrpriv->wowlan_war_offload_snmp_object_name, 0, MAX_SNMP_OBJECT_NAME_LEN);
        _rtw_memcpy(pwrpriv->wowlan_war_offload_snmp_object_name, object_name, sizeof(object_name));
        pwrpriv->wowlan_war_offload_snmp_object_name_len = (strlen(object_name) > MAX_SNMP_OBJECT_NAME_LEN)?MAX_SNMP_OBJECT_NAME_LEN:strlen(object_name);
        if(MAX_SNMP_OBJECT_NAME_LEN > pwrpriv->wowlan_war_offload_snmp_object_name_len) {
            RTW_INFO("SNMP object name = %s\n", pwrpriv->wowlan_war_offload_snmp_object_name);
        } else {
            RTW_INFO("SNMP object name is too long to print\n");
        }
    }
...
int proc_get_offload_snmp_sleep_status(struct seq_file *m, void *v)
{
	struct net_device *dev = m->private;
	_adapter *adapter = (_adapter *)rtw_netdev_priv(dev);
	struct dvobj_priv *dvobj = adapter_to_dvobj(adapter);
	...
	if (_TRUE == pwrpriv->wowlan_war_offload_mode) {
	    if ((WAR_SNMP_V4_RSP_EN & pwrpriv->wowlan_war_offload_ctrl) ||
                ...
                (WAR_SNMP_V4_WAKEUP_EN & pwrpriv->wowlan_war_offload_ctrl) ||
                (WAR_SNMP_V6_WAKEUP_EN & pwrpriv->wowlan_war_offload_ctrl)) {
		    RTW_PRINT_SEL(m, "\nSleep Status:%d\n", pwrpriv->wowlan_war_offload_snmp_sleep_status);
        } else {
	        RTW_PRINT_SEL(m, "\nSNMP Not enabled\n");
        }
	} else {
...
ssize_t proc_set_offload_snmp_sleep_status(struct file *file, const char __user *buffer, size_t count, loff_t *pos, void *data)
{
	struct net_device *dev = data;
	_adapter *padapter = (_adapter *)rtw_netdev_priv(dev);
	struct dvobj_priv *dvobj = adapter_to_dvobj(padapter);
	...
	if (buffer && !copy_from_user(tmp, buffer, count)) {
        int num = sscanf(tmp, "%d", &sleep_status);
        if(1 == num) {
            pwrpriv->wowlan_war_offload_snmp_sleep_status = sleep_status;
        }
    }
...
#endif /* CONFIG_OFFLOAD_SNMP_V4 || CONFIG_OFFLOAD_SNMP_V6 */

#ifdef CONFIG_WAKE_PATTERN_EN

int proc_get_wake_pattern_info(struct seq_file *m, void *v)
{
	struct net_device *dev = m->private;
	_adapter *padapter = (_adapter *)rtw_netdev_priv(dev);
	struct pwrctrl_priv *pwrpriv = adapter_to_pwrctl(padapter);
	struct wow_wake_pattern_priv *pwpriv = &pwrpriv->wpriv;
	...
	total = pwpriv->wowlan_wake_pattern_idx;

	/*show pattern*/
	RTW_PRINT_SEL(m, "\n======[Pattern Info.]======\n");
	RTW_PRINT_SEL(m, "wakeup pattern number: %d\n", total);
	RTW_PRINT_SEL(m, "wakeup pattern total size: %d\n", pwpriv->wowlan_wake_total_len);
	...
	for (k = 0; k < total ; k++) {
		...
		p_str = str_1;
		max_len = sizeof(str_1);
		for (i = 0 ; i < MAX_WAKE_PATTERN_SIZE / 8 ; i++) {
			_rtw_memset(p_str, 0, max_len);
			len = 0;
			for (j = 0 ; j < 8 ; j++) {
				val8 = pwpriv->wake_patterns[k].content[i * 8 + j];
		...
		RTW_PRINT_SEL(m, "\npattern mask:\n");
		for (i = 0 ; i < MAX_WAKE_SIZE / 8 ; i++) {
			_rtw_memset(p_str, 0, max_len);
			len = 0;
			for (j = 0 ; j < 8 ; j++) {
				val8 = pwpriv->wake_patterns[k].mask[i * 8 + j];
				len += snprintf(p_str + len, max_len - len,
						"%02x ", val8);
			}
			RTW_PRINT_SEL(m, "%s\n", p_str);
...
ssize_t proc_set_wake_pattern_info(struct file *file, const char __user *buffer, size_t count, loff_t *pos, void *data)
{
	struct net_device *dev = data;
	_adapter *padapter = (_adapter *)rtw_netdev_priv(dev);
	struct pwrctrl_priv *pwrpriv = adapter_to_pwrctl(padapter);
	...
	u8 tmp[(MAX_WAKE_PATTERN_SIZE*3)] = {0};
	int ret = 0, num = 0, pattern_len = 0;
	u8 index = 0;
	...
	if (count > sizeof(tmp)) {
		RTW_PRINT("WARNING: the length of input pattern is greater than %d.\n", MAX_WAKE_PATTERN_SIZE);
		/* rtw_warn_on(1); */
		return -EFAULT;
	}
	...
	if (buffer && !copy_from_user(tmp, buffer, count)) {
		if (strncmp(tmp, "clean", 5) == 0) {
			rtw_wow_wake_pattern_sw_reset(padapter);
		} else {

			if (pwpriv->wowlan_wake_pattern_idx >= MAX_WAKE_NUM) {
				RTW_PRINT("WARNING: priv-pattern is full(idx: %d)\n",
					 pwpriv->wowlan_wake_pattern_idx);
				RTW_PRINT("WARNING: please clean priv-pattern first\n");
				return -ENOMEM;
			}
				
			index = pwpriv->wowlan_wake_pattern_idx;
			ret = rtw_wow_wake_pattern_parser_pattern_cmd(tmp,
						pwpriv->wake_patterns[index].content,
						&pwpriv->wake_patterns[index].len,
						pwpriv->wake_patterns[index].mask);
			if (ret == _TRUE)
			{
				pattern_len = rtw_wow_wake_pattern_check_pattern_len(padapter);
				if(pattern_len > 0)
				{
					pwpriv->wowlan_wake_pattern_idx++;
					pwpriv->wowlan_wake_total_len += pattern_len;
				}	
			}
		}
	}

	RTW_INFO("%s : ret(%d) ==> pattern content(%d), len(%d), total len(%d).\n", __func__, ret, 
		pwpriv->wake_patterns[index].len, pattern_len, pwpriv->wowlan_wake_total_len);

	return count;
}

#endif /* CONFIG_WAKE_PATTERN_EN */
...
ssize_t proc_set_offload_mdns_machine_name(struct file *file, const char __user *buffer, size_t count, loff_t *pos, void *data)
{
	...
	struct dvobj_priv *dvobj = adapter_to_dvobj(padapter);
	struct pwrctrl_priv *pwrpriv = dvobj_to_pwrctl(dvobj);
	char tmp[MAX_MDNS_MACHINE_NAME_LEN*3+1];
	...
	if (count > sizeof(tmp)) {
		RTW_INFO(FUNC_ADPT_FMT ": input length, %zu, is large than MAX_MDNS_MACHINE_NAME_LEN(%d)\n", FUNC_ADPT_ARG(padapter), (count+1)/3, MAX_MDNS_MACHINE_NAME_LEN);
		rtw_warn_on(1);
		return -EFAULT;
	}
	...
	if (buffer && !copy_from_user(tmp, buffer, count)) {
        ...
        if( strncmp(tmp, "clean", 5) == 0 )
		{
			...
			if(rtw_wow_war_mdns_parser_pattern(tmp, pwrpriv->wowlan_war_offload_mdns_mnane[idx].name,
				&tmp_name_len, MAX_MDNS_MACHINE_NAME_LEN)) {
				pwrpriv->wowlan_war_offload_mdns_mnane[idx].name_len = (u8) tmp_name_len;
...
ssize_t proc_set_offload_mdns_service_info(struct file *file, const char __user *buffer, size_t count, loff_t *pos, void *data)
{
	...
	if (buffer && !copy_from_user(tmp, buffer, count)) {
		int num = sscanf(tmp, "%15s %4s %5s %63s %x %x %u %hhu", srv, trans, domain, target, &port0, &port1, &ttl, &txt_idx);
        ...
        {
            RTW_INFO(FUNC_ADPT_FMT ": Length of total parameters does not match the input buffer. (%d != %zu)\n",
        ...
        }else
		{
            ...
            {
                RTW_INFO(FUNC_ADPT_FMT ": input txt idx, %u, is out of range (0~%d)!\n", FUNC_ADPT_ARG(padapter), txt_idx, MAX_MDNS_TXT_NUM-1);
                count = -EFAULT;    
                goto exit;
            }
			...
			psinfo[idx].ttl = ttl;
			psinfo[idx].txt_rsp_idx = (s8)txt_idx;
...
ssize_t proc_set_offload_mdns_txt_rsp(struct file *file, const char __user *buffer, size_t count, loff_t *pos, void *data)
{
     ...
     if (buffer && !copy_from_user(tmp, buffer, count)) {
         ...
         if( strncmp(tmp, "clean", 5) == 0 )
         {
            ...
            /* set ==> */
            int num = sscanf(tmp, "%hhu %6s %256c", &idx, op, txt_str);
            ...
            {
                RTW_INFO(FUNC_ADPT_FMT ": Length of total parameters does not match the input buffer. (%d != %zu)(num=%d)\n",
                        FUNC_ADPT_ARG(padapter), sscanf_parameter_length, count, num);
                RTW_INFO(FUNC_ADPT_FMT ": Please check the content and length of each parameter.\n", FUNC_ADPT_ARG(padapter));
                RTW_INFO(FUNC_ADPT_FMT ": input buffer = (%s)(%zu)!\n\n", FUNC_ADPT_ARG(padapter), tmp, count);
                RTW_INFO(FUNC_ADPT_FMT ": op. = %s (%zu)!\n", FUNC_ADPT_ARG(padapter), op, strlen(op));
                RTW_INFO(FUNC_ADPT_FMT ": txt = %s (%zu)!\n", FUNC_ADPT_ARG(padapter), txt_str, strlen(txt_str));
                count = -EFAULT;    
                goto exit;
            }else
            {
                ...
                {
                    RTW_INFO(FUNC_ADPT_FMT ": the index, %u, is over the range of txt rsp(0~%d)!\n", FUNC_ADPT_ARG(padapter), idx, MAX_MDNS_TXT_NUM-1);
                    count = -EFAULT;    
                    goto exit; 
                }
                ...
                }else if(strncmp(op, "append", 6) == 0 )
                {
                    ...
                    {
                        RTW_INFO(FUNC_ADPT_FMT ": the txt rsp(%d) will be over the limitation(%d) if append input string(%zu)!\n", FUNC_ADPT_ARG(padapter),
...
#if defined(CONFIG_OFFLOAD_SSDP_V4) || defined(CONFIG_OFFLOAD_SSDP_V6)

int proc_get_offload_ssdp_pattern(struct seq_file *m, void *v)
{
	struct net_device *dev = m->private;
	_adapter *padapter = (_adapter *)rtw_netdev_priv(dev);
	struct pwrctrl_priv *pwrpriv = adapter_to_pwrctl(padapter);
	struct wow_ssdp_pattern_priv *pssdppriv = &pwrpriv->ssdppriv;
	struct registry_priv *pregistrypriv = &padapter->registrypriv;
	u8 pattern_num = 0, val8;
	char str_1[128];
	char *p_str;
	...
	p_str = str_1;
	max_len = sizeof(str_1);
	total = pssdppriv->wowlan_ssdp_pattern_idx;

	/*show pattern*/
	RTW_PRINT_SEL(m, "\n======[Pattern Info.]======\n");
	RTW_PRINT_SEL(m, "ssdp pattern number: %d\n", total);
	RTW_PRINT_SEL(m, "ssdp pattern total size: %d\n", pssdppriv->wowlan_ssdp_total_len);

	for (k = 0; k < total ; k++) {
		RTW_PRINT_SEL(m, "\npattern idx: %d\n", k);
		RTW_PRINT_SEL(m, "pattern content:\n");
		RTW_PRINT_SEL(m, "%s\n", pssdppriv->ssdp_patterns[k].content);
		p_str = str_1;
		max_len = sizeof(str_1);
		_rtw_memset(p_str, 0, max_len);
		len = 0;
		for (i = 0 ; i < pssdppriv->ssdp_patterns[k].len ; i++) {
			val8 = pssdppriv->ssdp_patterns[k].content[i];
			len += snprintf(p_str + len, max_len - len, "%02x", val8);
		}
		RTW_PRINT_SEL(m, "%s\n", p_str);
		RTW_PRINT_SEL(m, "\npriv_pattern_len:\n");
		RTW_PRINT_SEL(m, "pattern_len: %d\n", pssdppriv->ssdp_patterns[k].len);
		RTW_PRINT_SEL(m, "*****************\n");
	}
...
ssize_t proc_set_offload_ssdp_pattern(struct file *file, const char __user *buffer, size_t count, loff_t *pos, void *data)
{
	struct net_device *dev = data;
	_adapter *padapter = (_adapter *)rtw_netdev_priv(dev);
	struct pwrctrl_priv *pwrpriv = adapter_to_pwrctl(padapter);
	struct wow_ssdp_pattern_priv *pssdppriv = &pwrpriv->ssdppriv;
	u8 tmp[MAX_SSDP_PATTERN_SIZE] = {0};
	int ret = 0, num = 0, pattern_len = 0;
	u8 index = 0;
	...
	if (buffer && !copy_from_user(tmp, buffer, count)) {
		if (strncmp(tmp, "clean", 5) == 0) {
			rtw_wow_ssdp_pattern_sw_reset(padapter);
		} else {

			if (pssdppriv->wowlan_ssdp_pattern_idx >= MAX_SSDP_NUM) {
				RTW_PRINT("WARNING: priv-pattern is full(idx: %d)\n",
					 pssdppriv->wowlan_ssdp_pattern_idx);
				RTW_PRINT("WARNING: please clean priv-pattern first\n");
				return -ENOMEM;
			}
				
			index = pssdppriv->wowlan_ssdp_pattern_idx;
			ret = rtw_wow_ssdp_pattern_parser_pattern_cmd(tmp,
					count,
					pssdppriv->ssdp_patterns[index].content,
					&pssdppriv->ssdp_patterns[index].len);
			if (ret == _TRUE)
			{
				pattern_len = rtw_wow_ssdp_check_pattern_len(padapter);
				if(pattern_len > 0)
				{
					pssdppriv->wowlan_ssdp_pattern_idx++;
					pssdppriv->wowlan_ssdp_total_len += pattern_len;
				}	
			}				
		}
	}
	...
	RTW_INFO("%s : ret(%d) ==> pattern content(%d), len(%d), total len(%d).\n", __func__, ret, 
		pssdppriv->ssdp_patterns[index].len, pattern_len, pssdppriv->wowlan_ssdp_total_len);

	return count;
}

#endif /* defined(CONFIG_OFFLOAD_SSDP_V4) || defined(CONFIG_OFFLOAD_SSDP_V6) */

#if defined(CONFIG_OFFLOAD_WSD_V4) || defined(CONFIG_OFFLOAD_WSD_V6)

int proc_get_offload_wsd_pattern(struct seq_file *m, void *v)
{
	struct net_device *dev = m->private;
	_adapter *padapter = (_adapter *)rtw_netdev_priv(dev);
	struct pwrctrl_priv *pwrpriv = adapter_to_pwrctl(padapter);
	struct wow_wsd_pattern_priv *pwsdpriv = &pwrpriv->wsdpriv;
	struct registry_priv *pregistrypriv = &padapter->registrypriv;
	u8 pattern_num = 0, val8;
	char str_1[128];
	char *p_str;
	...
	p_str = str_1;
	max_len = sizeof(str_1);
	total = pwsdpriv->wowlan_wsd_pattern_idx;

	/*show pattern*/
	RTW_PRINT_SEL(m, "\n======[Pattern Info.]======\n");
	RTW_PRINT_SEL(m, "wsd pattern number: %d\n", total);
	RTW_PRINT_SEL(m, "wsd pattern total size: %d\n", pwsdpriv->wowlan_wsd_total_len);

	for (k = 0; k < total ; k++) {
		RTW_PRINT_SEL(m, "\npattern idx: %d\n", k);
		RTW_PRINT_SEL(m, "pattern content:\n");
		RTW_PRINT_SEL(m, "%s\n", pwsdpriv->wsd_patterns[k].content);
		p_str = str_1;
		max_len = sizeof(str_1);
		_rtw_memset(p_str, 0, max_len);
		len = 0;
		for (i = 0 ; i < pwsdpriv->wsd_patterns[k].len ; i++) {
			val8 = pwsdpriv->wsd_patterns[k].content[i];
			len += snprintf(p_str + len, max_len - len, "%02x", val8);
		}
		RTW_PRINT_SEL(m, "%s\n", p_str);
		RTW_PRINT_SEL(m, "\npriv_pattern_len:\n");
		RTW_PRINT_SEL(m, "pattern_len: %d\n", pwsdpriv->wsd_patterns[k].len);
		RTW_PRINT_SEL(m, "*****************\n");
	}
...
ssize_t proc_set_offload_wsd_pattern(struct file *file, const char __user *buffer, size_t count, loff_t *pos, void *data)
{
	struct net_device *dev = data;
	_adapter *padapter = (_adapter *)rtw_netdev_priv(dev);
	struct pwrctrl_priv *pwrpriv = adapter_to_pwrctl(padapter);
	struct wow_wsd_pattern_priv *pwsdpriv = &pwrpriv->wsdpriv;
	u8 tmp[MAX_WSD_PATTERN_SIZE] = {0};
	int ret = 0, num = 0, pattern_len = 0;
	u8 index = 0;
	...
	if (buffer && !copy_from_user(tmp, buffer, count)) {
		if (strncmp(tmp, "clean", 5) == 0) {
			rtw_wow_wsd_pattern_sw_reset(padapter);
		} else {

			if (pwsdpriv->wowlan_wsd_pattern_idx >= MAX_WSD_NUM) {
				RTW_PRINT("WARNING: priv-pattern is full(idx: %d)\n",
					 pwsdpriv->wowlan_wsd_pattern_idx);
				RTW_PRINT("WARNING: please clean priv-pattern first\n");
				return -ENOMEM;
			}
				
			index = pwsdpriv->wowlan_wsd_pattern_idx;
			ret = rtw_wow_wsd_pattern_parser_pattern_cmd(tmp,
					count,
					pwsdpriv->wsd_patterns[index].content,
					&pwsdpriv->wsd_patterns[index].len);
			if (ret == _TRUE)
			{
				pattern_len = rtw_wow_wsd_check_pattern_len(padapter);
				if(pattern_len > 0)
				{
					pwsdpriv->wowlan_wsd_pattern_idx++;
					pwsdpriv->wowlan_wsd_total_len += pattern_len;
				}	
			}				
		}
	}
	...
	RTW_INFO("%s : ret(%d) ==> pattern content(%d), len(%d), total len(%d).\n", __func__, ret, 
		pwsdpriv->wsd_patterns[index].len, pattern_len, pwsdpriv->wowlan_wsd_total_len);

	return count;
}

#endif /* defined(CONFIG_OFFLOAD_WSD_V4) || defined(CONFIG_OFFLOAD_WSD_V6) */

#if defined(CONFIG_OFFLOAD_SLP_V4) || defined(CONFIG_OFFLOAD_SLP_V6)

int proc_get_offload_slp_pattern(struct seq_file *m, void *v)
{
	struct net_device *dev = m->private;
	_adapter *padapter = (_adapter *)rtw_netdev_priv(dev);
	struct pwrctrl_priv *pwrpriv = adapter_to_pwrctl(padapter);
	struct wow_slp_pattern_priv *pslppriv = &pwrpriv->slppriv;
	struct registry_priv *pregistrypriv = &padapter->registrypriv;
	u8 pattern_num = 0, val8;
	char str_1[128];
	char *p_str;
	...
	p_str = str_1;
	max_len = sizeof(str_1);
	total = pslppriv->wowlan_slp_pattern_idx;

	/*show pattern*/
	RTW_PRINT_SEL(m, "\n======[Pattern Info.]======\n");
	RTW_PRINT_SEL(m, "slp pattern number: %d\n", total);
	RTW_PRINT_SEL(m, "slp pattern total size: %d\n", pslppriv->wowlan_slp_total_len);

	for (k = 0; k < total ; k++) {
		RTW_PRINT_SEL(m, "\npattern idx: %d\n", k);
		RTW_PRINT_SEL(m, "pattern content:\n");
		RTW_PRINT_SEL(m, "%s\n", pslppriv->slp_patterns[k].content);
		p_str = str_1;
		max_len = sizeof(str_1);
		_rtw_memset(p_str, 0, max_len);
		len = 0;
		for (i = 0 ; i < pslppriv->slp_patterns[k].len ; i++) {
			val8 = pslppriv->slp_patterns[k].content[i];
			len += snprintf(p_str + len, max_len - len, "%02x", val8);
		}
		RTW_PRINT_SEL(m, "%s\n", p_str);
		RTW_PRINT_SEL(m, "\npriv_pattern_len:\n");
		RTW_PRINT_SEL(m, "pattern_len: %d\n", pslppriv->slp_patterns[k].len);
		RTW_PRINT_SEL(m, "*****************\n");
	}
...
ssize_t proc_set_offload_slp_pattern(struct file *file, const char __user *buffer, size_t count, loff_t *pos, void *data)
{
	struct net_device *dev = data;
	_adapter *padapter = (_adapter *)rtw_netdev_priv(dev);
	struct pwrctrl_priv *pwrpriv = adapter_to_pwrctl(padapter);
	struct wow_slp_pattern_priv *pslppriv = &pwrpriv->slppriv;
	u8 tmp[MAX_SLP_PATTERN_SIZE] = {0};
	int ret = 0, num = 0, pattern_len = 0;
	u8 index = 0;
	...
	if (buffer && !copy_from_user(tmp, buffer, count)) {
		if (strncmp(tmp, "clean", 5) == 0) {
			rtw_wow_slp_pattern_sw_reset(padapter);
		} else {

			if (pslppriv->wowlan_slp_pattern_idx >= MAX_SLP_NUM) {
				RTW_PRINT("WARNING: priv-pattern is full(idx: %d)\n",
					 pslppriv->wowlan_slp_pattern_idx);
				RTW_PRINT("WARNING: please clean priv-pattern first\n");
				return -ENOMEM;
			}
				
			index = pslppriv->wowlan_slp_pattern_idx;
			ret = rtw_wow_slp_pattern_parser_pattern_cmd(tmp,
					count,
					pslppriv->slp_patterns[index].content,
					&pslppriv->slp_patterns[index].len);
			if (ret == _TRUE)
			{
				pattern_len = rtw_wow_slp_check_pattern_len(padapter);
				if(pattern_len > 0)
				{
					pslppriv->wowlan_slp_pattern_idx++;
					pslppriv->wowlan_slp_total_len += pattern_len;
				}	
			}				
		}
	}
	...
	RTW_INFO("%s : ret(%d) ==> pattern content(%d), len(%d), total len(%d).\n", __func__, ret, 
		pslppriv->slp_patterns[index].len, pattern_len, pslppriv->wowlan_slp_total_len);

	return count;
}

#endif /* defined(CONFIG_OFFLOAD_SLP_V4) || defined(CONFIG_OFFLOAD_SLP_V6) */
